...
// THEORY
//
//  Pathfinding in (c)mangos is based on detour recast an opensource nashmesh
//  creation and pathfinding codebase. This system is used for mob and npc
//  pathfinding and in this codebase also for bots. Because mobs and npc
//  movement is based on following a player or a set path the PathGenerator is
//  limited to 296y. This means that when trying to find a path from A to B
//  distances beyond 296y will be a best guess often moving in a straight path.
//  Bots would get stuck moving from Northshire to Stormwind because there is no
//  296y path that doesn't go (initially) the wrong direction.
//
//  To remedy this limitation without altering the PathGenerator limits too much
//  this node system was introduced.
//
//   <S> ---> [N1] ---> [N2] ---> [N3] ---> <E>
//
//  Bot at <S> wants to move to <E>
//  [N1],[N2],[N3] are predefined nodes for wich we know we can move from [N1]
//  to [N2] and from [N2] to [N3] but not from [N1] to [N3] If we can move fom
//  [S] to [N1] and from [N3] to [E] we have a complete route to travel.
//
//  Termonology:
//  Node: a location on a map for which we know bots are likely to want to
//  travel to or need to travel past to reach other nodes. Link: the connection
//  between two nodes. A link signifies that the bot can travel from one node to
//  another. A link is one-directional. Path: the waypointpath returned by the
//  standard PathGenerator to move from one node (or position) to another. A
//  path can be imcomplete or empty which means there is no link. Route: the
//  list of nodes that give the shortest route from a node to a distant node.
//  Routes are calculated using a standard A* search based on links.
//
//  On server start saved nodes and links are loaded. Paths and routes are
//  calculated on the fly but saved for future use. Nodes can be added and
//  removed realtime however because bots access the nodes from different
//  threads this requires a locking mechanism.
//
//  Initially the current nodes have been made:
//  Flightmasters and Inns (Bots can use these to fast-travel so eventually they
//  will be included in the route calculation) WorldBosses and Unique bosses in
//  instances (These are a logical places bots might want to go in instances)
//  Player start spawns (Obviously all lvl1 bots will spawn and move from here)
//  Area triggers locations with teleport and their teleport destinations (These
//  used to travel in or between maps) Transports including elevators (Again
//  used to travel in and in maps) (sub)Zone means (These are the center most
//  point for each sub-zone which is good for global coverage)
//
//  To increase coverage/linking extra nodes can be automatically be created.
//  Current implentation places nodes on paths (including complete) at sub-zone
//  transitions or randomly. After calculating possible links the node is
//  removed if it does not create local coverage.
//
...
public:
    ...
    // TravelNodePath(float distance1, float extraCost1, bool portal1 = false,
    // uint32 portalId1 = 0, bool transport1 = false, bool calculated = false,
    // uint8 maxLevelMob1 = 0, uint8 maxLevelAlliance1 = 0, uint8 maxLevelHorde1 =
    // 0, float swimDistance1 = 0, bool flightPath1 = false);
    ...
    // void setPortal(bool portal1, uint32 portalId1 = 0) { portal = portal1;
    // portalId = portalId1; } void setTransport(bool transport1) { transport =
    // transport1; }
...
public:
    ...
    // Removes links to other nodes that can also be reached by passing another
    // node.
