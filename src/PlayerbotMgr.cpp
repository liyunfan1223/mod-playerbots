...
void PlayerbotHolder::HandlePlayerBotLoginCallback(PlayerbotLoginQueryHolder const& holder)
{
    ...
    if (!bot)
    {
        ...
        delete botSession;
        // LOG_ERROR("playerbots", "Error logging in bot {}, please try to reset all
        // random bots", holder.GetGuid().ToString().c_str());
        return;
    }
    ...
    else
    {
        ...
        // LOG_ERROR("playerbots", "Attempt to add not allowed bot {}, please try to
        // reset all random bots", bot->GetName());
    }
}
...
void PlayerbotHolder::LogoutPlayerBot(ObjectGuid guid)
{
    if (Player* bot = GetPlayerBot(guid))
    {
        ...
        if (botAI->GetAiObjectContext())  // Maybe some day re-write to delate all
                                          // pointer values.
        {
            target = botAI->GetAiObjectContext()->GetValue<TravelTarget*>("travel target")->Get();
        }

        // Peiru: Allow bots to always instant logout to see if this resolves logout
        // crashes
        logout = true;
        ...
        if (!logout)
        {
            ...
            {
                playerBots.erase(guid);     // deletes bot player ptr inside this
                                            // WorldSession PlayerBotMap
        ...
        else if (bot && (logout || !botWorldSessionPtr->isLogingOut()))
        {
            ...
            botWorldSessionPtr->LogoutPlayer(true);  // this will delete the bot Player
                                                     // object and PlayerbotAI object
            delete botWorldSessionPtr;               // finally delete the bot's WorldSession
        }
    }
}
...
void PlayerbotHolder::OnBotLogin(Player* const bot)
{
    ...
    if (current_zone && cMgr)
    {
        for (uint32 i = 0; i < sChatChannelsStore.GetNumRows(); ++i)
        {
            ...
            // skip non built-in channels or global channel without zone name in
            // pattern
                                                     ...
                                                     bool admin, uint32 masterAccountId, uint32 masterGuildId)
{
    ...
    if (GET_PLAYERBOT_AI(bot))
    {
        ...
        if (cmd == "refresh=raid")
        {  // TODO: This function is not perfect yet. If
           // you are already in a raid,
            // after the command is executed, the AI ​​needs to go back online or
            // exit the raid and re-enter.
...
std::vector<std::string> PlayerbotHolder::HandlePlayerbotCommand(char const* args, Player* master)
{
    ...
    {
        messages.push_back(
            "usage: list/reload/tweak/self or add/init/remove "
            "PLAYERNAME or addclass CLASSNAME");
        return messages;
    }
    ...
    if (!strcmp(cmd, "addclass"))
    {
        ...
        if (!charname)
        {
            messages.push_back(
                "addclass: invalid "
                "CLASSNAME(warrior/paladin/hunter/rogue/priest/shaman/"
        ...
        QueryResult results = CharacterDatabase.Query(
            ...
            "WHERE name IN (SELECT name FROM playerbots_names) AND class = '{}' "
            "AND online = 0 AND race IN ({}) AND guid NOT IN ( SELECT guid FROM "
            "guild_member ) "
            "AND account <= {} "
    ...
    if (!charname)
    {
        ...
        // Player* tPlayer =
        // ObjectAccessor::FindConnectedPlayer(master->GetTarget());
        if (isPlayer)
        {
            charnameStr = name;
        }
