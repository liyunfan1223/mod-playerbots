      ...
      security(bot)  // reorder args - whipowill
{
    ...
    // masterIncomingPacketHandlers.AddHandler(CMSG_GAMEOBJ_USE, "use game
    // object");
...
void PlayerbotAI::UpdateAI(uint32 elapsed, bool minimal)
{
    ...
    // }
    // if (!bot->HasUnitMovementFlag(MOVEMENTFLAG_FALLING) && bot->GetPositionZ()
    // - bot->GetFloorZ() > 0.1f) {
    ...
    //     bot->Say("falling... flag: " +
    //     std::to_string(bot->GetUnitMovementFlags()), LANG_UNIVERSAL);
    // }
    // bot->SendMovementFlagUpdate();
    // float x, y, z;
    ...
    switch (packet.GetOpcode())
    {
        ...
        case SMSG_MOVE_KNOCK_BACK:  // handle knockbacks
        {
            ...
            bot->MovePositionToFirstCollision(
                dest, dist, bot->GetRelativeAngle(bot->GetPositionX() + vcos, bot->GetPositionY() + vsin));
            float x, y, z;
            x = dest.GetPositionX();
            y = dest.GetPositionY();
            z = dest.GetPositionZ();
            ...
            // verticalSpeed: %.2f, tX: %.2f, tY: %.2f, tZ: %.2f, relativeAngle: %.2f,
            // orientation: %.2f",
            //     vcos, vsin, horizontalSpeed, verticalSpeed, x, y, z,
            //     bot->GetRelativeAngle(vcos, vsin), bot->GetOrientation());
            // bot->Say(speak, LANG_UNIVERSAL);
            // bot->GetClosePoint(x, y, z, bot->GetObjectSize(), dist,
            // bot->GetAngle(vcos, vsin));
            ...
            // bot->GetMotionMaster()->MoveKnockbackFrom(fx, fy, horizontalSpeed,
            // verticalSpeed);

            // // set delay based on actual distance
            // float newdis = sqrt(sServerFacade->GetDistance2d(bot, fx, fy));
            // SetNextCheckDelay((uint32)((newdis / dis) * moveTimeHalf * 4 *
            // IN_MILLISECONDS));
    ...
    if (currentEngine != engine)
    {
        ...
        switch (type)
        {
            ...
            case BOT_STATE_NON_COMBAT:
                // LOG_DEBUG("playerbots",  "=== {} NON-COMBAT ===",
                // bot->GetName().c_str());
                break;
            case BOT_STATE_DEAD:
...
void PlayerbotAI::DoNextAction(bool min)
{
    ...
    /*
        ...
        if (bot->m_movementInfo.HasMovementFlag(MOVEMENTFLAG_FALLING))
        {
            ...
            bot->m_movementInfo.pos = !GetJumpDestination().m_positionZ == 0 ?
       GetJumpDestination() : bot->GetPosition(); bot->m_movementInfo.jump =
...
Unit* PlayerbotAI::GetUnit(CreatureData const* creatureData)
{
    ...
    if (!spawnId)  // workaround for CreatureData with missing spawnId (this just
                   // uses first matching creatureId in DB, but thats ok this
                   // method is only used for battlemasters and theres only 1 of
                   // each type)
...
//     map->GetGameObjectBySpawnIdStore().equal_range(gameObjectData->spawnId);
...
bool PlayerbotAI::CanCastSpell(uint32 spellid, Unit* target, bool checkHasSpell, Item* itemTarget, Item* castItem)
{
    ...
    if (bot->HasUnitState(UNIT_STATE_LOST_CONTROL))
    {
        if (!sPlayerbotAIConfig->logInGroupOnly || (bot->GetGroup() && HasRealPlayerMaster()))
        {
            LOG_DEBUG("playerbots",
                      "Can cast spell failed. Unit state lost control. - spellid: "
                      "{}, bot name: {}",
                      spellid, bot->GetName());
        }
        return false;
    ...
    // HasRealPlayerMaster()))
    //     LOG_DEBUG("playerbots", "Can cast spell? - target name: {}, spellid:
    //     {}, bot name: {}",
    ...
    if (checkHasSpell && !bot->HasSpell(spellid))
    {
        ...
        {
            LOG_DEBUG("playerbots",
                      "Can cast spell failed. Bot not has spell. - target name: {}, "
                      "spellid: {}, bot name: {}",
                      target->GetName(), spellid, bot->GetName());
        }
        return false;
    ...
    if (bot->GetCurrentSpell(CURRENT_CHANNELED_SPELL) != nullptr)
    {
        ...
        {
            LOG_DEBUG("playerbots",
                      "CanCastSpell() target name: {}, spellid: {}, bot name: {}, "
                      "failed because has current channeled spell",
                      target->GetName(), spellid, bot->GetName());
        }
        return false;
    }
    ...
    if (bot->HasSpellCooldown(spellid))
    {
        ...
        {
            LOG_DEBUG("playerbots",
                      "Can cast spell failed. Spell not has cooldown. - target name: "
                      "{}, spellid: {}, bot name: {}",
                      target->GetName(), spellid, bot->GetName());
        }
        return false;
    ...
    if (!spellInfo)
    {
        ...
        {
            LOG_DEBUG("playerbots",
                      "Can cast spell failed. No spellInfo. - target name: {}, "
                      "spellid: {}, bot name: {}",
                      target->GetName(), spellid, bot->GetName());
        }
        return false;
    ...
    if (CastingTime && bot->isMoving())
    {
        ...
        {
            LOG_DEBUG("playerbots",
                      "Casting time and bot is moving - target name: {}, spellid: "
                      "{}, bot name: {}",
                      target->GetName(), spellid, bot->GetName());
        }
        return false;
    ...
    if (!itemTarget)
    {
        ...
        if (target->IsImmunedToSpell(spellInfo))
        {
            ...
            {
                LOG_DEBUG("playerbots",
                          "target is immuned to spell - target name: {}, spellid: {}, "
                          "bot name: {}",
                          target->GetName(), spellid, bot->GetName());
            }
            return false;
        ...
        //             if (!sPlayerbotAIConfig->logInGroupOnly || (bot->GetGroup()
        //             && HasRealPlayerMaster())) {
        //                 LOG_DEBUG("playerbots", "target is immuned to spell
        //                 effect - target name: {}, spellid: {}, bot name: {}",
        //                     target->GetName(), spellid, bot->GetName());
        //             }
        //             return false;
        //         }
        ...
        if (bot != target && sServerFacade->GetDistance2d(bot, target) > sPlayerbotAIConfig->sightDistance)
        {
            ...
            {
                LOG_DEBUG("playerbots",
                          "target is out of sight distance - target name: {}, spellid: "
                          "{}, bot name: {}",
                          target->GetName(), spellid, bot->GetName());
            }
            return false;
    ...
    //         LOG_DEBUG("playerbots", "CanCastSpell - target name: {}, spellid:
    //         {}, bot name: {}, result: {}",
    ...
    switch (result)
    {
        ...
        default:
            if (!sPlayerbotAIConfig->logInGroupOnly || (bot->GetGroup() && HasRealPlayerMaster()))
            {
                // if (result != SPELL_FAILED_NOT_READY && result != SPELL_CAST_OK) {
                LOG_DEBUG("playerbots",
                          "CanCastSpell Check Failed. - target name: {}, spellid: {}, "
                          "bot name: {}, result: {}",
                          target->GetName(), spellid, bot->GetName(), result);
            }
            return false;
...
bool PlayerbotAI::CastSpell(uint32 spellId, Unit* target, Item* itemTarget)
{
    ...
    if (bot->IsFlying() || bot->HasUnitState(UNIT_STATE_IN_FLIGHT))
    {
        // if (!sPlayerbotAIConfig->logInGroupOnly || (bot->GetGroup() &&
        // HasRealPlayerMaster())) {
        //     LOG_DEBUG("playerbots", "Spell cast is flying - target name: {},
        //     spellid: {}, bot name: {}}",
        //         target->GetName(), spellId, bot->GetName());
        // }
        return false;
    ...
    if (spellInfo->Targets & TARGET_FLAG_ITEM)
    {
        ...
        if (bot->GetTradeData())
        {
            ...
            // if (!sPlayerbotAIConfig->logInGroupOnly || (bot->GetGroup() &&
            // HasRealPlayerMaster())) {
            //     LOG_DEBUG("playerbots", "Spell cast no item - target name: {},
            //     spellid: {}, bot name: {}",
            //         target->GetName(), spellId, bot->GetName());
            // }
            return true;
    ...
    {
        // WorldLocation aoe = aiObjectContext->GetValue<WorldLocation>("aoe
        // position")->Get(); targets.SetDst(aoe);
    ...
    // if (!sPlayerbotAIConfig->logInGroupOnly || (bot->GetGroup() &&
    // HasRealPlayerMaster())) {
    //     LOG_DEBUG("playerbots", "Spell cast result - target name: {}, spellid:
    //     {}, bot name: {}, result: {}",
    ...
    if (result != SPELL_CAST_OK)
    {
        // if (!sPlayerbotAIConfig->logInGroupOnly || (bot->GetGroup() &&
        // HasRealPlayerMaster())) {
        //     LOG_DEBUG("playerbots", "Spell cast failed. - target name: {},
        //     spellid: {}, bot name: {}, result: {}",
        //         target->GetName(), spellId, bot->GetName(), result);
        // }
        return false;
    }
    ...
    // {
    //     LootObject loot = *aiObjectContext->GetValue<LootObject>("loot
    //     target"); if (!loot.IsLootPossible(bot))
    //     {
    //         spell->cancel();
    //         delete spell;
    //         if (!sPlayerbotAIConfig->logInGroupOnly || (bot->GetGroup() &&
    //         HasRealPlayerMaster())) {
    //             LOG_DEBUG("playerbots", "Spell cast loot - target name: {},
    //             spellid: {}, bot name: {}",
    //                 target->GetName(), spellId, bot->GetName());
    //         }
    //         return false;
...
bool PlayerbotAI::CastVehicleSpell(uint32 spellId, Unit* target)
{
    ...
    if (seat->CanControl())
    {
        // aiObjectContext->GetValue<LastMovement&>("last
        // movement")->Get().Set(nullptr); aiObjectContext->GetValue<time_t>("stay
        // time")->Set(0);
    }
    ...
    // aiObjectContext->GetValue<LastSpellCast&>("last spell
    // cast")->Get().Set(spellId, target->GetGUID(), time(0));
...
uint32 PlayerbotAI::GetFixedBotNumer(BotTypeNumber typeNumber, uint32 maxNum, float cyclePerMin)
{
    ...
    randnum = (randnum % (maxNum + 1));  // Loops the randomnumber at maxNum. Bassically
                                         // removes all the numbers above 99.
    return randnum;                      // Now we have a number unique for each bot between 0 and
                                         // maxNum that increases by cyclePerMin.
}
...
bool PlayerbotAI::AllowActive(ActivityType activityType)
{
    ...
    // The default is 10%. With 0.1% of all bots going active or inactive each
    // minute.
    ...
    return ActivityNumber <=
           (sPlayerbotAIConfig->botActiveAlone * mod) / 100;  // The given percentage of bots should be active and
                                                              // rotate 1% of those active bots each minute.
}
...
uint32 PlayerbotAI::GetMixedGearScore(Player* player, bool withBags, bool withBank, uint32 topN)
{
    ...
    if (!topN)
    {
        ...
        if (gearScore[EQUIPMENT_SLOT_MAINHAND] + gearScore[EQUIPMENT_SLOT_OFFHAND] < twoHandScore * 2)
        {
            gearScore[EQUIPMENT_SLOT_OFFHAND] = 0;  // off hand is ignored in calculations if 2h
                                                    // weapon has higher score
            --count;
            gearScore[EQUIPMENT_SLOT_MAINHAND] = twoHandScore;
        }
                                     ...
                                     bool mixed)
{
    ...
    switch (type)
    {
        ...
        case INVTYPE_HANDS:
            (*gearScore)[EQUIPMENT_SLOT_HEAD] = std::max((*gearScore)[EQUIPMENT_SLOT_HEAD], level);
            break;
        // equipped gear score check uses both rings and trinkets for calculation,
        // assume that for bags/banks it is the same with keeping second highest score
        // at second slot
        case INVTYPE_FINGER:
        {
...
/**
 * FindStoneFor()
 * return Item* Returns sharpening/weight stone item eligible to enchant a bot
 * weapon
 *
 * params:weapon Item* the weap�n the function should search and return a
 * enchanting item for return nullptr if no relevant item is found in bot
 * inventory, else return a sharpening or weight stone based on the weapon
 * subclass
 *
 */
...
void PlayerbotAI::EnchantItemT(uint32 spellid, uint8 slot)
{
    ...
    if (!enchantid)
    {
        // LOG_ERROR("playerbots", "{}: Invalid enchantid ", enchantid, " report to
        // devs", bot->GetName().c_str());
        return;
    }
    ...
    {
        // LOG_ERROR("playerbots", "{}: items could not be enchanted, wrong item
        // type equipped", bot->GetName().c_str());
        return;
    }
