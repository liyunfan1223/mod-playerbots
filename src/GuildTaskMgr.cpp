...
void GuildTaskMgr::Update(Player* player, Player* guildMaster)
{
        ...
        (secLevel == PLAYERBOT_SECURITY_TALK && reason != PLAYERBOT_DENY_FAR))
    {
        LOG_DEBUG("playerbots",
                  "{} / {}: skipping guild task update - not enough security "
                  "level, reason = {}",
...
bool GuildTaskMgr::CreateKillTask(Player* player, uint32 guildId)
{
    ...
    QueryResult results = WorldDatabase.Query(
        "SELECT ct.Entry, c.map, c.position_x, c.position_y, ct.Name FROM "
        "creature_template ct "
        "JOIN creature c ON ct.Entry = c.id1 WHERE ct.MaxLevel < {} AND "
        "ct.MinLevel > {} AND ct.Rank = {} ",
                                         ...
                                         uint32 guildId, uint32 validIn)
{
    ...
    body << "As you probably know " << proto->Name
         << " is wanted dead for the crimes it did against our guild. If you "
            "should kill it ";
...
bool GuildTaskMgr::Reward(CharacterDatabaseTransaction& trans, uint32 owner, uint32 guildId)
{
    ...
    if (itemTask)
    {
        ...
        body << "We wish to thank you for the " << proto->Name1
             << " you provided so kindly. We really appreciate this and may this "
                "small gift bring you our thanks!\n";
        body << "\n";
        body << "Many thanks,\n";
        body << guild->GetName() << "\n";
        body << leader->GetName() << "\n";
    ...
    else if (killTask)
    {
        ...
        body << "We wish to thank you for the " << proto->Name
             << " you've killed recently. We really appreciate this and may this "
                "small gift bring you our thanks!\n";
        body << "\n";
        body << "Many thanks,\n";
        body << guild->GetName() << "\n";
...
void GuildTaskMgr::CleanupAdverts()
{
    ...
    QueryResult result = CharacterDatabase.Query(
        "SELECT id, receiver FROM mail WHERE subject "
        "LIKE 'Guild Task%%' AND deliver_time <= {}",
        deliverTime);
    if (!result)
        return;
    ...
    if (count > 0)
    {
        CharacterDatabase.Execute(
            "DELETE FROM mail WHERE subject LIKE 'Guild "
            "Task%%' AND deliver_time <= {}",
            deliverTime);
...
void GuildTaskMgr::RemoveDuplicatedAdverts()
{
    ...
    QueryResult result = CharacterDatabase.Query(
        "SELECT m.id, m.receiver FROM (SELECT MAX(id) AS id, subject, receiver "
        "FROM mail WHERE subject LIKE 'Guild Task%%' "
        "AND deliver_time <= {} GROUP BY subject, receiver) q JOIN mail m ON "
        "m.subject = q.subject WHERE m.id <> q.id AND m.deliver_time <= {}",
