...
void RandomItemMgr::BuildItemInfoCache()
{
    ...
    for (auto const& itr : *itemTemplate)
    {
        ...
        //     if (!((1 << (clazz - 1)) & CLASSMASK_ALL_PLAYABLE) ||
        //     !sChrClassesStore.LookupEntry(clazz))
        //         continue;
        ...
        //         if ((proto->Class == ITEM_CLASS_WEAPON || (proto->SubClass ==
        //         ITEM_SUBCLASS_ARMOR_SHIELD || (proto->SubClass ==
        //         ITEM_SUBCLASS_ARMOR_MISC && proto->InventoryType ==
        //         INVTYPE_HOLDABLE))) &&
        ...
        //         // set stat weight = 1 for items that can be equipped but have no
        //         proper stats statWeight.weight = statW ? statW : 1;
        //         //statWeight.weight = statW;
        //         // save item statWeight into ItemCache
        //         cacheInfo.weights[statWeight.id] = statWeight.weight;
        //         LOG_DEBUG("playerbots", "Item: {}, weight: {}, class: {}, spec:
        //         {}", proto->ItemId, statWeight.weight, clazz,
        //         m_weightScales[clazz][spec].info.name);
        //     }
        // }
        ...
        // if (cacheInfo.team == TEAM_NEUTRAL && proto->AllowableRace > 1 &&
        // proto->AllowableRace < 8388607)
        // {
        ...
        //     LOG_DEBUG("playerbots", "Item: {}, team (item): {}", proto->ItemId,
        //     cacheInfo.team == TEAM_ALLIANCE ? "Alliance" : "Horde");
        ...
        //     LOG_DEBUG("playerbots", "Item: {}, source: PvP Reward",
        //     proto->ItemId);
        // }
        ...
        //         bool isAlly = false;
        //         bool isHorde = false;
        //         for (std::vector<uint32>::iterator i = questIds.begin(); i !=
        //         questIds.end(); ++i)
        //         {
        ...
        //         LOG_DEBUG("playerbots", "Item: {}, team (quest): {}",
        //         proto->ItemId, cacheInfo.team == TEAM_ALLIANCE ? "Alliance" :
        //         cacheInfo.team == TEAM_HORDE ? "Horde" : "Both");
        //         LOG_DEBUG("playerbots", "Item: {}, source: quest {}, minlevel:
        //         {}", proto->ItemId, cacheInfo.sourceId, cacheInfo.minLevel);
        //     }
        // }

        // if (cacheInfo.minLevel)
        //     LOG_DEBUG("playerbots", "Item: {}, minlevel: {}", proto->ItemId,
        //     cacheInfo.minLevel);

        // // check vendors
        // if (cacheInfo.source == ITEM_SOURCE_NONE)
        // {
        //     for (std::set<uint32>::iterator i = vendorItems.begin(); i !=
        //     vendorItems.end(); ++i)
        //     {
        //         if (proto->ItemId == *i)
        //         {
        //             cacheInfo.source = ITEM_SOURCE_VENDOR;
        //             LOG_DEBUG("playerbots", "Item: {} source: vendor",
        //             proto->ItemId); break;
        //         }
        //     }
        // }
        ...
        //             LOG_DEBUG("playerbots", "Item: {}, source: creature drop, ID:
        //             {}", proto->ItemId, creatures.front());
        //         }
        //         else
        //         {
        //             cacheInfo.source = ITEM_SOURCE_DROP;
        //             LOG_DEBUG("playerbots", "Item: {}, source: creatures drop,
        //             number: {}", proto->ItemId, creatures.size());
        //         }
        //     }
        // }

        // // check gameobject drop
        // if (cacheInfo.source == ITEM_SOURCE_NONE || (cacheInfo.source ==
        // ITEM_SOURCE_DROP && !cacheInfo.sourceId))
        // {
        //     if (gameobjects.size())
        //     {
        ...
        //             LOG_INFO("playerbots", "Item: {}, source: gameobject, ID:
        //             {}", proto->ItemId, gameobjects.front());
        //         }
        //         else
        //         {
        //             cacheInfo.source = ITEM_SOURCE_DROP;
        //             LOG_INFO("playerbots", "Item: {}, source: gameobjects,
        //             number: {}", proto->ItemId, gameobjects.size());
        //         }
        //     }
        // }
        ...
        // proto->RequiredReputationFaction != 35 && proto->RequiredReputationRank <
        // 15)
        // {
        ...
        // // save cache
        // PlayerbotsDatabasePreparedStatement* stmt =
        // PlayerbotsDatabase.GetPreparedStatement(PLAYERBOTS_DEL_EQUIP_CACHE_NEW);
        // stmt->SetData(0, proto->ItemId);
        // trans->Append(stmt);

        // stmt =
        // PlayerbotsDatabase.GetPreparedStatement(PLAYERBOTS_INS_EQUIP_CACHE_NEW);
    ...
    for (const auto& spellData : proto->Spells)
    {
        ...
        for (uint32 j = 0; j < MAX_SPELL_EFFECTS; j++)
        {
            if (spellInfo->Effects[j].Effect == SPELL_EFFECT_APPLY_AURA && spellInfo->Effects[j].BasePoints)
            {
                ...
                //{
                //    statWeight += CalculateSingleStatWeight(playerclass, spec,
                //    "block", spellInfo->Effects[j].BasePoints + 1);
                //}
...
uint32 RandomItemMgr::CalculateSingleStatWeight(uint8 playerclass, uint8 spec, std::string stat, uint32 value)
{
         ...
         i != m_weightScales[playerclass][spec].stats.end(); ++i)
    {
        if (stat == i->stat)
        {
            ...
            // if (statWeight)
            //     LOG_INFO("playerbots", "stat: {}, val: {}, weight: {}, total: {},
            //     class: {}, spec: {}",
            //         stat, value, i->weight, statWeight, playerclass,
            //         m_weightScales[playerclass][spec].info.name);
            return statWeight;
        }
    }
                                                  ...
                                                  uint32 itemId, uint32 amount)
{
    ...
    for (std::map<uint32, ItemInfoEntry>::iterator i = itemInfoCache.begin(); i != itemInfoCache.end(); ++i)
    {
        ...
        //     uint32 bestSpecId = 0;
        //     uint32 bestSpecScore = 0;
        //     for (vector<uint32>::iterator i = classspecs.begin(); i !=
        //     classspecs.end(); ++i)
        //     {
        //         if (info->weights[*i] > bestSpecScore)
        //         {
...
uint32 RandomItemMgr::GetLiveStatWeight(Player* player, uint32 itemId)
{
    ...
    /*uint32 bestSpecId = 0;
    uint32 bestSpecScore = 0;
    for (vector<uint32>::iterator i = classspecs.begin(); i != classspecs.end();
    ++i)
    {
        if (itemCache[itemId]->weights[*i] > bestSpecScore &&
    itemCache[itemId]->weights[*i] > 1)
        {
            bestSpecId = *i;
...
void RandomItemMgr::BuildEquipCache()
{
    ...
    if (PreparedQueryResult result = PlayerbotsDatabase.Query(stmt))
    {
        LOG_INFO("server.loading",
                 "Loading equipment cache for {} classes, {} levels, {} slots, {} "
                 "quality from {} items",
    ...
    else
    {
        ...
        LOG_INFO("server.loading",
                 "Building equipment cache for {} classes, {} levels, {} slots, {} "
                 "quality from {} items ({} total)",
                ...
                for (uint8 slot = 0; slot < EQUIPMENT_SLOT_END; ++slot)
                {
                    for (uint32 quality = ITEM_QUALITY_POOR; quality <= ITEM_QUALITY_ARTIFACT; ++quality)
                    {
                        ...
                        LOG_DEBUG("playerbots",
                                  "Equipment cache for class: {}, level {}, slot {}, "
                                  "quality {}: {} items",
    ...
    for (uint32 level = 1; level <= maxLevel; level += 1)
    {
        for (uint32 subClass = ITEM_SUBCLASS_ARROW; subClass <= ITEM_SUBCLASS_BULLET; subClass++)
        {
            QueryResult results = WorldDatabase.Query(
                "SELECT entry, Flags FROM item_template WHERE class = {} AND "
                "subclass = {} AND RequiredLevel <= {} AND stackable = 1000 "
    ...
    else
    {
        ...
        for (auto const& itr : *itemTemplates)
        {
            ...
            QueryResult results = WorldDatabase.Query(
                ...
                "    ELSE   "
                "    IFNULL(100 - (SELECT SUM(ChanceOrQuestChance) FROM "
                "creature_loot_template lt1 WHERE lt1.groupid = lt.groupid AND "
                "lt1.entry = lt.entry AND lt1.ChanceOrQuestChance > 0), 100) "
                "    / (SELECT COUNT(*) FROM creature_loot_template lt1 WHERE "
                "lt1.groupid = lt.groupid AND lt1.entry = lt.entry AND "
                "lt1.ChanceOrQuestChance = 0) "
                "    END "
                ...
                "    ELSE   "
                "    IFNULL(100 - (SELECT SUM(ChanceOrQuestChance) FROM "
                "gameobject_loot_template lt1 WHERE lt1.groupid = lt.groupid AND "
                "lt1.entry = lt.entry AND lt1.ChanceOrQuestChance > 0), 100) "
                "    / (SELECT COUNT(*) FROM gameobject_loot_template lt1 WHERE "
                "lt1.groupid = lt.groupid AND lt1.entry = lt.entry AND "
                "lt1.ChanceOrQuestChance = 0) "
                "    END "
                ...
                "    ELSE   "
                "    IFNULL(100 - (SELECT SUM(ChanceOrQuestChance) FROM "
                "disenchant_loot_template lt1 WHERE lt1.groupid = lt.groupid AND "
                "lt1.entry = lt.entry AND lt1.ChanceOrQuestChance > 0), 100) "
                "    / (SELECT COUNT(*) FROM disenchant_loot_template lt1 WHERE "
                "lt1.groupid = lt.groupid AND lt1.entry = lt.entry AND "
                "lt1.ChanceOrQuestChance = 0) "
                "    END "
                ...
                "    ELSE   "
                "    IFNULL(100 - (SELECT SUM(ChanceOrQuestChance) FROM "
                "fishing_loot_template lt1 WHERE lt1.groupid = lt.groupid AND "
                "lt1.entry = lt.entry AND lt1.ChanceOrQuestChance > 0), 100) "
                "    / (SELECT COUNT(*) FROM fishing_loot_template lt1 WHERE "
                "lt1.groupid = lt.groupid AND lt1.entry = lt.entry AND "
                "lt1.ChanceOrQuestChance = 0) "
                "    END "
                ") chance, 'fishing' type "
                ...
                "    ELSE   "
                "    IFNULL(100 - (SELECT SUM(ChanceOrQuestChance) FROM "
                "skinning_loot_template lt1 WHERE lt1.groupid = lt.groupid AND "
                "lt1.entry = lt.entry AND lt1.ChanceOrQuestChance > 0), 100) "
                "    * IFNULL((SELECT 1/COUNT(*) FROM skinning_loot_template lt1 "
                "WHERE lt1.groupid = lt.groupid AND lt1.entry = lt.entry AND "
                "lt1.ChanceOrQuestChance = 0), 1) "
                "    END "
