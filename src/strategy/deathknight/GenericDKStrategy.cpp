...
private:
    ...
    static ActionNode* killing_machine([[maybe_unused]] PlayerbotAI* botAI)
    {
        return new ActionNode("killing machine",
                              /*P*/ nullptr,
                              /*A*/
                              NextAction::array(0, new NextAction("improved icy talons"), nullptr),
                              /*C*/ nullptr);
    }
...
void GenericDKStrategy::InitTriggers(std::vector<TriggerNode*>& triggers)
{
    ...
    // triggers.push_back(new TriggerNode("high aoe", NextAction::array(0, new
    // NextAction("anti magic shell", ACTION_NORMAL + 3), nullptr)));
    // triggers.push_back(new TriggerNode("death coil", NextAction::array(0, new
    // NextAction("death coil", ACTION_NORMAL + 3), nullptr)));
    // triggers.push_back(new TriggerNode("critical aoe heal",
    // NextAction::array(0, new NextAction("anti magic zone", ACTION_EMERGENCY +
    // 1), nullptr)));
    triggers.push_back(
    ...
    // NextAction::array(0, new NextAction("reach melee", ACTION_NORMAL + 8),
    // nullptr)));
    triggers.push_back(
