...
UnholyDKStrategy::UnholyDKStrategy(PlayerbotAI *botAI) : GenericDKStrategy(botAI) { actionNodeFactories.Add(new UnholyDKStrategyActionNodeFactory()); }

NextAction **UnholyDKStrategy::getDefaultActions()
{
    return NextAction::array(0, new NextAction("death and decay", ACTION_DEFAULT + 1.0f), new NextAction("scourge strike", ACTION_DEFAULT + 0.8f), new NextAction("horn of winter", ACTION_DEFAULT + 0.6f),
                             new NextAction("summon gargoyle", ACTION_DEFAULT + 0.4f), new NextAction("death coil", ACTION_DEFAULT + 0.2f), new NextAction("melee", ACTION_DEFAULT), nullptr);
}
...
void UnholyDKStrategy::InitTriggers(std::vector<TriggerNode *> &triggers)
{
    ...
    // triggers.push_back(new TriggerNode("often", NextAction::array(0, new NextAction(,
    // ACTION_NORMAL + 2), nullptr)));
    ...
    triggers.push_back(
        new TriggerNode("death and decay cooldown", NextAction::array(0, new NextAction("ghoul frenzy", ACTION_DEFAULT + 5.0f), new NextAction("scourge strike", ACTION_DEFAULT + 4.0f), new NextAction("blood boil", ACTION_NORMAL + 3.0f),
                                                                      new NextAction("icy touch", ACTION_NORMAL + 2.0f), new NextAction("plague strike", ACTION_NORMAL + 1.0f), nullptr)));
}
...
void UnholyDKAoeStrategy::InitTriggers(std::vector<TriggerNode *> &triggers)
{
    ...
    triggers.push_back(new TriggerNode("medium aoe", NextAction::array(0, new NextAction("death and decay", ACTION_NORMAL + 3), new NextAction("corpse explosion", ACTION_NORMAL + 3), nullptr)));
}
