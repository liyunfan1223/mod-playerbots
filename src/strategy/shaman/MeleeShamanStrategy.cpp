...
{
    return NextAction::array(0, new NextAction("stormstrike", ACTION_DEFAULT + 0.5f),
...
void MeleeShamanStrategy::InitTriggers(std::vector<TriggerNode *> &triggers)
{
    ...
    triggers.push_back(
        new TriggerNode("main hand weapon no imbue",
    ...
    triggers.push_back(
        new TriggerNode("maelstrom weapon",
    ...
    triggers.push_back(new TriggerNode(
        "medium aoe",
        NextAction::array(0, new NextAction("strength of earth totem", ACTION_LIGHT_HEAL),
                          nullptr)));
    triggers.push_back(new TriggerNode(
        "enemy out of melee",
    ...
    triggers.push_back(new TriggerNode(
        "no fire totem", NextAction::array(0, new NextAction("reach melee", 23.0f),
                                           new NextAction("magma totem", 22.0f), nullptr)));

    triggers.push_back(
        new TriggerNode("fire elemental totem",
                        NextAction::array(0, new NextAction("reach melee", 33.0f),
                                          new NextAction("fire elemental totem", 32.0f), nullptr)));

    triggers.push_back(new TriggerNode(
        "no air totem", NextAction::array(0, new NextAction("windfury totem", 20.0f), nullptr)));

    triggers.push_back(new TriggerNode(
        "medium mana", NextAction::array(0, new NextAction("shamanistic rage", 23.0f), nullptr)));
}
...
void MeleeAoeShamanStrategy::InitTriggers(std::vector<TriggerNode *> &triggers)
{
    ...
    // totem", 26.0f), nullptr)));
    triggers.push_back(new TriggerNode(
        "medium aoe", NextAction::array(0, new NextAction("fire nova", 25.0f), nullptr)));
}
