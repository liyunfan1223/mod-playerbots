...
MeleeShamanStrategy::MeleeShamanStrategy(PlayerbotAI *botAI) : GenericShamanStrategy(botAI) { actionNodeFactories.Add(new MeleeShamanStrategyActionNodeFactory()); }

NextAction **MeleeShamanStrategy::getDefaultActions()
{
    return NextAction::array(0, new NextAction("stormstrike", ACTION_DEFAULT + 0.5f), new NextAction("earth shock", ACTION_DEFAULT + 0.4f), new NextAction("feral spirit", ACTION_DEFAULT + 0.3f), new NextAction("fire nova", ACTION_DEFAULT + 0.2f),
...
void MeleeShamanStrategy::InitTriggers(std::vector<TriggerNode *> &triggers)
{
    ...
    // triggers.push_back(new TriggerNode("shaman weapon", NextAction::array(0, new
    // NextAction("flametongue weapon", 22.0f), nullptr)));
    ...
    // NextAction("reach melee", 22.0f), new NextAction("searing totem", 22.0f), nullptr)));
    triggers.push_back(new TriggerNode("flame shock", NextAction::array(0, new NextAction("flame shock", 20.0f), nullptr)));
    triggers.push_back(new TriggerNode("maelstrom weapon", NextAction::array(0, new NextAction("lightning bolt", 25.0f), nullptr)));
    ...
    triggers.push_back(new TriggerNode("no fire totem", NextAction::array(0, new NextAction("reach melee", 23.0f), new NextAction("magma totem", 22.0f), nullptr)));

    triggers.push_back(new TriggerNode("fire elemental totem", NextAction::array(0, new NextAction("reach melee", 33.0f), new NextAction("fire elemental totem", 32.0f), nullptr)));

    triggers.push_back(new TriggerNode("no air totem", NextAction::array(0, new NextAction("windfury totem", 20.0f), nullptr)));
...
{
    // triggers.push_back(new TriggerNode("magma totem", NextAction::array(0, new NextAction("magma
    // totem", 26.0f), nullptr)));
