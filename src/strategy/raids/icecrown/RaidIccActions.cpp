...
bool IccDbsTankPositionAction::CrowdControlBloodBeasts()
{
    ...
    [[maybe_unused]] bool appliedCC = false;
    ...
    if (rangedIt != rangedDpsGuids.end())
    {
        ...
        if (rangedIndex < remainingSpotsInHealerRows)
        {
            ...
            else
            {
                ...
                [[maybe_unused]] int spotsInFirstRow = 6;
...
bool IccFestergutSporeAction::Execute([[maybe_unused]] Event event)
{
    ...
    [[maybe_unused]] constexpr float SPREAD_RADIUS = 2.0f;
...
bool IccRotfaceTankPositionAction::PositionMainTankAndMelee(Unit* boss)
{
    ...
    if (boss && boss->HasUnitState(UNIT_STATE_CASTING) && boss->GetCurrentSpell(SPELL_SLIME_SPRAY))
        [[maybe_unused]] bool isBossCasting = true;
...
bool IccRotfaceTankPositionAction::HandleBigOozePositioning([[maybe_unused]] Unit* boss)
{
    // Find all big oozes
...
bool IccRotfaceGroupPositionAction::Execute([[maybe_unused]] Event event)
{
    ...
    const GuidVector npcs = AI_VALUE(GuidVector, "nearest hostile npcs");
    [[maybe_unused]] bool floodPresent = false;
    ...
    [[maybe_unused]] Unit* bigOoze = AI_VALUE2(Unit*, "find target", "big ooze");
...
bool IccRotfaceGroupPositionAction::MoveAwayFromPuddle(Unit* boss, Unit* puddle, [[maybe_unused]] float puddleDistance)
{
    if (!boss || !puddle)
        return false;
...
bool IccRotfaceGroupPositionAction::FindAndMoveFromClosestMember([[maybe_unused]] Unit* boss, Unit* smallOoze)
{
    ...
    [[maybe_unused]] const bool isRanged = botAI->IsRanged(bot) || botAI->IsHeal(bot);
...
bool IccRotfaceMoveAwayFromExplosionAction::MoveToRandomSafeLocation()
{
    ...
    [[maybe_unused]] float currentZ = bot->GetPositionZ();
...
Unit* IccPutricideGrowingOozePuddleAction::FindClosestThreateningPuddle()
{
    ...
    [[maybe_unused]] float closestSafeDistance = BASE_RADIUS;
...
Position IccPutricideGasCloudAction::CalculateEmergencyPosition(const Position& botPos, float dx, float dy)
{
    ...
    [[maybe_unused]] static const float MOVEMENT_INCREMENT = 5.0f;  // Fixed movement increment
...
bool IccPutricideAvoidMalleableGooAction::HandleTankPositioning([[maybe_unused]] Unit* boss)
{
    if (!botAI->IsTank(bot))
        return false;
...
{
    [[maybe_unused]] static constexpr float MAX_HEIGHT_DIFF = 20.0f;
    static constexpr std::array<uint32_t, 4> KINETIC_BOMB_ENTRIES = {NPC_KINETIC_BOMB1, NPC_KINETIC_BOMB2,
                                                                     NPC_KINETIC_BOMB3, NPC_KINETIC_BOMB4};
...
bool IccBqlGroupPositionAction::HandleShadowsMovement()
{
    ...
    for (int curveIdx = 0; curveIdx < MAX_CURVES; curveIdx++)
    {
        ...
        [[maybe_unused]] float safeMoveTargetDist = FLT_MAX;
        ...
        if (foundSafe)
        {
            ...
            [[maybe_unused]] float stepsToCurve = minDist / 2.0f;  // Approximate steps to reach the curve
...
bool IccBqlGroupPositionAction::HandleGroupPosition(Unit* boss, Aura* frenzyAura, Aura* shadowAura)
{
    ...
    if (isRanged)
    {
        ...
        [[maybe_unused]] size_t totalRanged = leftSide.size() + rightSide.size() + rangedBots.size();
...
bool IccValithriaDreamCloudAction::Execute([[maybe_unused]] Event event)
{
    ...
    if (it == dreamBots.end())
        return false;
    [[maybe_unused]] size_t myIndex = std::distance(dreamBots.begin(), it);
...
bool IccSindragosaFrostBombAction::Execute([[maybe_unused]] Event event)
{
    ...
    [[maybe_unused]] ObjectGuid losTombGuid = myTombGuids[bestIdx];
    ...
    if (inLOS)
    {
        ...
        else
        {
            ...
            [[maybe_unused]] Unit* boss = AI_VALUE2(Unit*, "find target", "sindragosa");
            bot->AttackStop();
            return true;
        }
    }
...
bool IccLichKingWinterAction::Execute([[maybe_unused]] Event event)
{
    ...
    if (Group* group = bot->GetGroup())
    {
        ...
        if (!currentSkullTarget.IsEmpty())
        {
            [[maybe_unused]] Unit* skullTarget = ObjectAccessor::GetUnit(*bot, currentSkullTarget);
...
bool IccLichKingWinterAction::IsPositionSafeFromDefile(float x, float y, [[maybe_unused]] float z, float minSafeDistance)
{
    Unit* boss = AI_VALUE2(Unit*, "find target", "the lich king");
    if (!boss)
...
void IccLichKingWinterAction::HandleMainTankAddManagement([[maybe_unused]] Unit* boss, const Position* tankPos)
{
    if (!botAI->IsMainTank(bot))
        return;
...
void IccLichKingWinterAction::HandleAssistTankAddManagement([[maybe_unused]] Unit* boss, const Position* tankPos)
{
    if (!botAI->IsAssistTank(bot))
        return;
...
bool IccLichKingAddsAction::Execute([[maybe_unused]] Event event)
{
    ...
    [[maybe_unused]] Unit* spiritWarden = AI_VALUE2(Unit*, "find target", "spirit warden");
    bool hasPlague = botAI->HasAura("Necrotic Plague", bot);
...
void IccLichKingAddsAction::HandleShamblingHorrors([[maybe_unused]] Unit* boss, bool hasPlague)
{
    // Find closest shambling horror
...
bool IccLichKingAddsAction::HandleAssistTankAddManagement(Unit* boss, Difficulty diff)
{
    ...
    else
    {
        ...
        {
            [[maybe_unused]] Unit* mainTank = AI_VALUE(Unit*, "main tank");
...
void IccLichKingAddsAction::HandleDefileMechanics(Unit* boss, Difficulty diff)
{
    ...
    [[maybe_unused]] Unit* closestDefile = nullptr;
...
void IccLichKingAddsAction::HandleValkyrMarking(const std::vector<Unit*>& grabbingValkyrs, Difficulty diff)
{
    ...
    [[maybe_unused]] static constexpr const char* ICON_NAMES[] = {"skull", "cross", "star"};
