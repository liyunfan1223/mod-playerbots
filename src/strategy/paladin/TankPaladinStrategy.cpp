...
TankPaladinStrategy::TankPaladinStrategy(PlayerbotAI *botAI) : GenericPaladinStrategy(botAI) { actionNodeFactories.Add(new TankPaladinStrategyActionNodeFactory()); }

NextAction **TankPaladinStrategy::getDefaultActions()
{
    return NextAction::array(0, new NextAction("shield of righteousness", ACTION_DEFAULT + 0.6f), new NextAction("hammer of the righteous", ACTION_DEFAULT + 0.5f), new NextAction("judgement of wisdom", ACTION_DEFAULT + 0.4f),
...
void TankPaladinStrategy::InitTriggers(std::vector<TriggerNode *> &triggers)
{
    ...
    // triggers.push_back(new TriggerNode("devotion aura", NextAction::array(0, new
    // NextAction("devotion aura", 90.0f), NULL)));

    triggers.push_back(new TriggerNode("light aoe", NextAction::array(0, new NextAction("avenger's shield", ACTION_HIGH + 5), nullptr)));
    triggers.push_back(new TriggerNode("medium aoe", NextAction::array(0, new NextAction("melee consecration", ACTION_HIGH + 7), new NextAction("avenger's shield", ACTION_HIGH + 6), nullptr)));
    // triggers.push_back(new TriggerNode("avenger's shield", NextAction::array(0, new
    // NextAction("avenger's shield", ACTION_HIGH + 7), nullptr)));
    ...
    // NextAction("holy shield", ACTION_HIGH + 4), nullptr)));
    ...
    triggers.push_back(new TriggerNode("medium group heal occasion", NextAction::array(0, new NextAction("divine sacrifice", ACTION_HIGH + 5), nullptr)));
    triggers.push_back(new TriggerNode("enough mana", NextAction::array(0, new NextAction("melee consecration", ACTION_HIGH + 4), nullptr)));
