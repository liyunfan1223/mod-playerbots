...
void DpsPaladinStrategy::InitTriggers(std::vector<TriggerNode*>& triggers)
{
    ...
    // 3), new NextAction("holy light", ACTION_CRITICAL_HEAL + 2), nullptr)));
    // triggers.push_back(new TriggerNode("judgement of wisdom", NextAction::array(0, new
    // NextAction("judgement of wisdom", ACTION_NORMAL + 10), nullptr))); triggers.push_back(new
    // TriggerNode("judgement", NextAction::array(0, new NextAction("judgement", ACTION_HIGH + 10),
    // nullptr))); triggers.push_back(new TriggerNode("enemy is close", NextAction::array(0, new
    // NextAction("consecration", ACTION_INTERRUPT), nullptr))); triggers.push_back(new
    // TriggerNode("repentance on enemy healer", NextAction::array(0, new NextAction("repentance on
    // enemy healer", ACTION_INTERRUPT + 2), nullptr))); triggers.push_back(new
    // TriggerNode("repentance on snare target", NextAction::array(0, new NextAction("repentance on
