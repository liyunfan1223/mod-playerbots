...
void HealPaladinStrategy::InitTriggers(std::vector<TriggerNode *> &triggers)
{
    ...
    triggers.push_back(new TriggerNode(
        "seal", NextAction::array(0, new NextAction("seal of wisdom", ACTION_HIGH), nullptr)));
    triggers.push_back(new TriggerNode(
        "medium mana",
        NextAction::array(0, new NextAction("divine illumination", ACTION_HIGH + 2), nullptr)));
    triggers.push_back(new TriggerNode(
        "low mana",
    ...
    triggers.push_back(new TriggerNode(
        ...
        NextAction::array(0, new NextAction("reach party member to heal", ACTION_EMERGENCY + 3),
                          nullptr)));
    ...
    triggers.push_back(new TriggerNode(
        ...
        NextAction::array(0, new NextAction("holy light on party", ACTION_MEDIUM_HEAL + 5),
                          nullptr)));
    ...
    triggers.push_back(new TriggerNode(
        ...
        NextAction::array(0, new NextAction("flash of light on party", ACTION_LIGHT_HEAL + 3),
                          nullptr)));

    triggers.push_back(new TriggerNode(
        "beacon of light on main tank",
        NextAction::array(
            0, new NextAction("beacon of light on main tank", ACTION_CRITICAL_HEAL + 7), nullptr)));

    triggers.push_back(new TriggerNode(
        "sacred shield on main tank",
        NextAction::array(0, new NextAction("sacred shield on main tank", ACTION_CRITICAL_HEAL + 6),
                          nullptr)));
}
