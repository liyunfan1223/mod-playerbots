...
void WorldPacketHandlerStrategy::InitTriggers(std::vector<TriggerNode *> &triggers)
{
    ...
    triggers.push_back(new TriggerNode("levelup", NextAction::array(0, new NextAction("auto teleport for level", relevance + 3), new NextAction("auto talents", relevance + 2), new NextAction("auto learn spell", relevance + 1),
...
void ReadyCheckStrategy::InitTriggers(std::vector<TriggerNode *> &triggers) { triggers.push_back(new TriggerNode("timer", NextAction::array(0, new NextAction("ready check", relevance), nullptr))); }
