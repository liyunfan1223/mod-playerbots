...
BattleBotPath vPath_AV_Icewing_Bunker_Crossroad_to_Frostdagger_Pass_Lower = {
    ...
    // anymore as they get
    // captain down in first wave since uneeded dismounting was fixed) { 68.793f,
    // -396.742f, 45.299f, nullptr },
                ...
                if (!BgObjective && alterValleyBG->IsCaptainAlive(TEAM_ALLIANCE))
                {
                    if (Creature* enemyCaptain = bg->GetBGCreature(AV_CREATURE_A_CAPTAIN))
                    {
                        if (enemyCaptain->getDeathState() != DeathState::Dead)
                        {
                            ...
                            uint32 attackCount = getDefendersCount(AV_STONEHEARTH_WAITING_HORDE, 10.0f, false) +
                ...
                if (!BgObjective && alterValleyBG->IsCaptainAlive(TEAM_HORDE))
                {
                    if (Creature* enemyCaptain = bg->GetBGCreature(AV_CREATURE_H_CAPTAIN))
                    {
                        if (enemyCaptain->getDeathState() != DeathState::Dead)
                        {
                            BgObjective = enemyCaptain;
                            uint32 attackCount =
                                getDefendersCount(AV_ICEBLOOD_GARRISON_WAITING_ALLIANCE, 10.0f, false) +
                                getDefendersCount(AV_ICEBLOOD_GARRISON_ATTACKING_ALLIANCE, 10.0f, false);
                            // prepare to attack Captain
                            if (attackCount < 15 && !enemyCaptain->IsInCombat())
                            {
                                // get in position to attack Captain
                               ...
                               // points. Afterwards pick random strategies
        {
                ...
                else if (role < 8)  // Attack and Defend
                {
                    while (BgObjective == nullptr)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                if (eyeOfTheStormBG->GetCapturePointInfo(attackObjectivesBack[0])._ownerTeamId !=
                                    rootTeamIndex)
                                {
                                    BgObjective = bg->GetBGObject(attackObjectivesBack[0]);
                                }
                                else if (eyeOfTheStormBG->GetCapturePointInfo(attackObjectivesBack[1])._ownerTeamId !=
                                         rootTeamIndex)
                                {
                                    BgObjective = bg->GetBGObject(attackObjectivesBack[1]);
                                }
            ...
            if (bot->HasAura(BG_EY_NETHERSTORM_FLAG_SPELL))
            {
                ...
                else
                {
                    ...
                    if (bot->IsWithinDist3d(flagDeliverPoint.GetPositionX(), flagDeliverPoint.GetPositionY(),
                ...
                if (!MoveTo(bot->GetMapId(), flagDeliverPoint.GetPositionX(), flagDeliverPoint.GetPositionY(),
                            flagDeliverPoint.GetPositionZ()))
                {
...
bool BGTactics::selectObjectiveWp(std::vector<BattleBotPath*> const& vPaths)
{
    ...
    float botDistanceScoreMultiply = 3.0f;  // path score modifier - higher = less likely to chose a further path (it's
                                            // basically a multiplier on distance from bot - makes distance from bot
                                            // more signifcant than distance from destination)
