...
std::vector<Position> const WS_FLAG_HIDE_HORDE = {WS_FLAG_HIDE_HORDE_1, WS_FLAG_HIDE_HORDE_2,
                                                  WS_FLAG_HIDE_HORDE_3};
std::vector<Position> const WS_FLAG_HIDE_ALLIANCE = {
    WS_FLAG_HIDE_ALLIANCE_1, WS_FLAG_HIDE_ALLIANCE_2, WS_FLAG_HIDE_ALLIANCE_3};
...
std::vector<uint32> const vFlagsIC = {BG_IC_GO_HORDE_BANNER,    BG_IC_GO_ALLIANCE_BANNER,
                                      BG_IC_GO_WORKSHOP_BANNER, BG_IC_GO_DOCKS_BANNER,
                                      BG_IC_GO_HANGAR_BANNER,   BG_IC_GO_QUARRY_BANNER,
...
BattleBotPath vPath_AV_Icewing_Bunker_Crossroad_to_Frostdagger_Pass_Lower = {
    ...
    {119.693f, -351.311f, 42.728f, nullptr},  {107.710f, -321.162f, 37.168f, nullptr},
    {84.953f, -273.434f, 23.944f, nullptr},   {47.073f, -245.696f, 15.417f, nullptr},
    {2.455f, -240.346f, 11.867f, nullptr},    {-38.451f, -231.493f, 10.284f, nullptr},
    {-70.606f, -230.758f, 9.919f, nullptr},   {-83.955f, -214.666f, 12.873f, nullptr},
    {-111.412f, -175.575f, 13.515f, nullptr}, {-129.634f, -141.359f, 20.921f, nullptr},
    {-140.904f, -105.104f, 28.097f, nullptr}, {-167.552f, -74.891f, 31.524f, nullptr},
    {-210.442f, -54.664f, 37.525f, nullptr},  {-247.372f, -38.489f, 36.438f, nullptr},
    {-280.423f, -48.095f, 32.709f, nullptr},  {-309.757f, -58.032f, 28.778f, nullptr},
...
static uint32 AB_AttackObjectives[] = {
    // Attack
    BG_AB_NODE_STABLES, BG_AB_NODE_BLACKSMITH, BG_AB_NODE_FARM, BG_AB_NODE_LUMBER_MILL,
    BG_AB_NODE_GOLD_MINE};

static uint32 EY_AttackObjectives[] = {POINT_FEL_REAVER, POINT_BLOOD_ELF, POINT_DRAENEI_RUINS,
                                       POINT_MAGE_TOWER};
...
std::string const BGTactics::HandleConsoleCommandPrivate(WorldSession *session, char const *args)
{
    ...
    if (!strncmp(cmd, "showpath", 8))
    {
        ...
        {
            BattleBotWaypoint &waypoint = ((*path)[i]);
            Creature *wpCreature = player->SummonCreature(15631, waypoint.x, waypoint.y, waypoint.z,
                                                          0, TEMPSUMMON_TIMED_DESPAWN, 15000u);
    ...
    if (!strncmp(cmd, "showcreature=", 13))
    {
        ...
        Creature *wpCreature =
            player->SummonCreature(15631, c->GetPositionX(), c->GetPositionY(), c->GetPositionZ(),
                                   0, TEMPSUMMON_TIMED_DESPAWN, 15000u);
        wpCreature->SetOwnerGUID(player->GetGUID());
    ...
    if (!strncmp(cmd, "showobject=", 11))
    {
        ...
        Creature *wpCreature =
            player->SummonCreature(15631, o->GetPositionX(), o->GetPositionY(), o->GetPositionZ(),
                                   0, TEMPSUMMON_TIMED_DESPAWN, 15000u);
        wpCreature->SetOwnerGUID(player->GetGUID());
...
bool BGTactics::wsgPaths()
{
    ...
    if (pos.x > bot->GetPositionX())  // He's somewhere at the alliance side
    {
        ...
        else
        {
            ...
            else if (bot->GetPositionX() < 1031.f)  // to the first step of the ramp from the tunnel
            {
                ...
                return true;
                     ...
                     bot->GetPositionY() <
                         1494.f)  // to the second step of the ramp from the tunnel
            {
        ...
        if (Preference < 5)  // through the tunnel
        {
            ...
            {
                MoveTo(bg->GetMapId(), 1351.759155f + frand(0, 2), 1462.368042f + frand(-2, +2),
                       323.673737f);
                return true;
            }
...
bool BGTactics::moveToStart(bool force)
{
    ...
    if (bgType == BATTLEGROUND_WS)
    {
        ...
        int startSpot = role < 4   ? BB_WSG_WAIT_SPOT_LEFT
                        : role > 6 ? BB_WSG_WAIT_SPOT_RIGHT
                                   : BB_WSG_WAIT_SPOT_SPAWN;
        if (startSpot == BB_WSG_WAIT_SPOT_RIGHT)
        {
            if (bot->GetTeamId() == TEAM_HORDE)
                ...
                MoveTo(bg->GetMapId(),
                       WS_WAITING_POS_ALLIANCE_1.GetPositionX() + frand(-2.0f, 2.0f),
        ...
        else if (startSpot == BB_WSG_WAIT_SPOT_LEFT)
        {
            if (bot->GetTeamId() == TEAM_HORDE)
                ...
                MoveTo(bg->GetMapId(),
                       WS_WAITING_POS_ALLIANCE_2.GetPositionX() + frand(-2.0f, 2.0f),
    ...
    else if (bgType == BATTLEGROUND_IC)
    {
        ...
        if (bot->GetTeamId() == TEAM_HORDE)
        {
            if (role < 3)
            {
                if (urand(0, 1))
                    MoveTo(bg->GetMapId(),
                           IC_WEST_WAITING_POS_HORDE.GetPositionX() + frand(-5.0f, 5.0f),
            ...
            else
            {
                if (urand(0, 1))
                    MoveTo(bg->GetMapId(),
                           IC_EAST_WAITING_POS_HORDE.GetPositionX() + frand(-5.0f, 5.0f),
        ...
        else
        {
            if (role < 3)
            {
                if (urand(0, 1))
                    MoveTo(bg->GetMapId(),
                           IC_SIDE_WAITING_POS_ALLIANCE.GetPositionX() + frand(-5.0f, 5.0f),
                           IC_SIDE_WAITING_POS_ALLIANCE.GetPositionY() + frand(-5.0f, 5.0f),
                           IC_SIDE_WAITING_POS_ALLIANCE.GetPositionZ());
                else
                    MoveTo(bg->GetMapId(),
                           IC_WAITING_POS_ALLIANCE.GetPositionX() + frand(-5.0f, 5.0f),
...
bool BGTactics::selectObjective(bool reset)
{
    ...
    switch (bgType)
    {
        case BATTLEGROUND_AV:
        {
            ...
            uint32 role = context->GetValue<uint32>("bg role")->Get();
            bool supportDefense =
                role < 3;  // defensive role and mine capture (mine capture disabled for now)
            bool advancedAttack =
                role > 5;  // doesnt wait for point to be fully captured before moving on
            ...
            if (bot->GetTeamId() == TEAM_HORDE)
            {
                bool enemyTowersDown =
                    alterValleyBG->GetAVNodeInfo(BG_AV_NODES_DUNBALDAR_NORTH).State ==
                        POINT_DESTROYED &&
                    alterValleyBG->GetAVNodeInfo(BG_AV_NODES_DUNBALDAR_SOUTH).State ==
                        POINT_DESTROYED &&
                    alterValleyBG->GetAVNodeInfo(BG_AV_NODES_ICEWING_BUNKER).State ==
                        POINT_DESTROYED &&
                    alterValleyBG->GetAVNodeInfo(BG_AV_NODES_STONEHEART_BUNKER).State ==
                        POINT_DESTROYED;
                // End Boss
                if (enemyTowersDown &&
                    alterValleyBG->GetAVNodeInfo(BG_AV_NODES_FIRSTAID_STATION).OwnerId !=
                        TEAM_ALLIANCE &&
                    alterValleyBG->GetAVNodeInfo(BG_AV_NODES_FIRSTAID_STATION).TotalOwnerId !=
                        TEAM_ALLIANCE)
                {
                ...
                if (!BgObjective && supportDefense)
                {
                    ...
                    {
                        if (alterValleyBG->GetAVNodeInfo(objective.first).State !=
                                POINT_DESTROYED &&
                ...
                if (!BgObjective && alterValleyBG->IsCaptainAlive(TEAM_ALLIANCE))
                {
                    if (Creature *enemyCaptain = bg->GetBGCreature(AV_CREATURE_A_CAPTAIN))
                    {
                        if (enemyCaptain->getDeathState() != DeathState::Dead)
                        {
                            ...
                            uint32 attackCount =
                                getDefendersCount(AV_STONEHEARTH_WAITING_HORDE, 10.0f, false) +
                                getDefendersCount(AV_STONEHEARTH_ATTACKING_HORDE, 10.0f, false);
                ...
                if (!BgObjective && supportDefense && alterValleyBG->IsCaptainAlive(TEAM_HORDE))
                {
                    ...
                    {
                        if (myCaptain->getDeathState() != DeathState::Dead &&
                            bot->IsWithinDist(myCaptain, 200.f))
                ...
                if (!BgObjective && supportDefense)
                {
                    for (const auto &objective :
                         AV_HordeDefendObjectives)  // TODO go reverse instead of not breaking
                    {
                ...
                if (false && !BgObjective && supportDefense &&
                    ...
                    alterValleyBG->GetAVNodeInfo(BG_AV_NODES_STORMPIKE_GRAVE).OwnerId !=
                        TEAM_ALLIANCE)
                {
                    if (Creature *mBossNeutral = bg->GetBGCreature(AV_CPLACE_MINE_N_3))
                    {
                            ...
                            alterValleyBG->GetMineOwner(AV_NORTH_MINE) == TEAM_NEUTRAL)
                        {
                            ...
                            LOG_INFO("playerbots", "HORDE bot={} Attacking Neutral Mine Boss!",
                                     bot->GetName());
                    ...
                    if (Creature *mBossAlly = bg->GetBGCreature(AV_CPLACE_MINE_N_3))
                    {
                            ...
                            alterValleyBG->GetMineOwner(AV_NORTH_MINE) == TEAM_NEUTRAL)
                        {
                            ...
                            LOG_INFO("playerbots", "HORDE bot={} Attacking Ally Mine Boss!",
                                     bot->GetName());
                            // std::ostringstream out;
                ...
                if (!BgObjective &&
                    alterValleyBG->GetAVNodeInfo(AV_HordeAttackObjectives[0].first).OwnerId ==
                        TEAM_ALLIANCE)
                {
                ...
                if (!BgObjective)
                {
                    ...
                    {
                        if ((!advancedAttack &&
                             alterValleyBG->GetAVNodeInfo(objective.first).TotalOwnerId ==
                                 TEAM_ALLIANCE) ||
                            (alterValleyBG->GetAVNodeInfo(objective.first).OwnerId ==
                             TEAM_ALLIANCE))  // OwnerId changes when objective partially captured,
            ...
            {
                bool enemyTowersDown =
                    alterValleyBG->GetAVNodeInfo(BG_AV_NODES_FROSTWOLF_WTOWER).State ==
                        POINT_DESTROYED &&
                    alterValleyBG->GetAVNodeInfo(BG_AV_NODES_FROSTWOLF_ETOWER).State ==
                        POINT_DESTROYED &&
                    alterValleyBG->GetAVNodeInfo(BG_AV_NODES_TOWER_POINT).State ==
                        POINT_DESTROYED &&
                    alterValleyBG->GetAVNodeInfo(BG_AV_NODES_ICEBLOOD_TOWER).State ==
                        POINT_DESTROYED;
                // End Boss
                if (enemyTowersDown &&
                    alterValleyBG->GetAVNodeInfo(BG_AV_NODES_FROSTWOLF_HUT).OwnerId != TEAM_HORDE &&
                    alterValleyBG->GetAVNodeInfo(BG_AV_NODES_FROSTWOLF_HUT).TotalOwnerId !=
                        TEAM_HORDE)
                {
                ...
                //  (supportDefense) Only go to Snowfall Graveyard if already close to it.
                if (!BgObjective && supportDefense &&
                    alterValleyBG->GetAVNodeInfo(BG_AV_NODES_SNOWFALL_GRAVE).OwnerId !=
                        TEAM_ALLIANCE)
                {
                    if (GameObject *pGO = bg->GetBGObject(BG_AV_OBJECT_FLAG_N_SNOWFALL_GRAVE))
                        if (bot->IsWithinDist(pGO, 100.f))
                ...
                if (!BgObjective && alterValleyBG->IsCaptainAlive(TEAM_HORDE))
                {
                    if (Creature *enemyCaptain = bg->GetBGCreature(AV_CREATURE_H_CAPTAIN))
                    {
                        if (enemyCaptain->getDeathState() != DeathState::Dead)
                        {
                            BgObjective = enemyCaptain;
                            uint32 attackCount =
                                getDefendersCount(AV_ICEBLOOD_GARRISON_WAITING_ALLIANCE, 10.0f,
                                                  false) +
                                getDefendersCount(AV_ICEBLOOD_GARRISON_ATTACKING_ALLIANCE, 10.0f,
                                                  false);
                            // prepare to attack Captain
                            if (attackCount < 15 && !enemyCaptain->IsInCombat())
                            {
                ...
                {
                    for (const auto &objective :
                         AV_AllianceDefendObjectives)  // TODO go reverse instead of not breaking
                    {
                        if (alterValleyBG->GetAVNodeInfo(objective.first).State != POINT_DESTROYED)
                        {
                            if (GameObject *pGO = bg->GetBGObject(objective.second))
                ...
                if (false && !BgObjective && supportDefense &&
                    ...
                    alterValleyBG->GetAVNodeInfo(BG_AV_NODES_FROSTWOLF_GRAVE).TotalOwnerId !=
                        TEAM_HORDE)
                {
                    if (Creature *mBossNeutral = bg->GetBGCreature(AV_CPLACE_MINE_S_3))
                    {
                        ...
                        {
                            BgObjective = mBossNeutral;
                            LOG_INFO("playerbots", "ALLIANCE bot={} Attacking Neutral Mine Boss!",
                                     bot->GetName());
                            // std::ostringstream out;
                            // out << "Attacking Neutral Mine Boss!";
                            // bot->Say(out.str(), LANG_UNIVERSAL);
                    ...
                    if (Creature *mBossHorde = bg->GetBGCreature(AV_CPLACE_MINE_S_3))
                    {
                            ...
                            alterValleyBG->GetMineOwner(AV_SOUTH_MINE) == TEAM_HORDE)
                        {
                            ...
                            LOG_INFO("playerbots", "ALLIANCE bot={} Attacking Horde Mine Boss!",
                                     bot->GetName());
                            // std::ostringstream out;
                ...
                // toward getting GY first)
                if (!BgObjective &&
                    alterValleyBG->GetAVNodeInfo(AV_AllianceAttackObjectives[0].first).OwnerId ==
                        TEAM_HORDE)
                {
                    ...
                    for (const auto &objective : AV_AllianceAttackObjectives)
                    {
                        if ((!advancedAttack &&
                             alterValleyBG->GetAVNodeInfo(objective.first).TotalOwnerId ==
                                 TEAM_HORDE) ||
                            (alterValleyBG->GetAVNodeInfo(objective.first).OwnerId ==
                             TEAM_HORDE))  // OwnerId changes when objective partially captured,
                                           // TotalOwnerId only changes when its fully
                                           // captured/destroyed (so this code will allow
                                           // advancedAttack bots move on to next point)
                        {
            ...
            {
                if (!pos.isSet())
                    pos.Set(BgObjective->GetPositionX(), BgObjective->GetPositionY(),
                            BgObjective->GetPositionZ(), BgObjective->GetMapId());
            ...
            if (bot->HasAura(BG_WS_SPELL_WARSONG_FLAG) || bot->HasAura(BG_WS_SPELL_SILVERWING_FLAG))
            {
                if (bot->GetTeamId() == TEAM_ALLIANCE)
                {
                    if (teamFlagTaken())
                    {
                        ...
                        pos.Set(hidePos.GetPositionX(), hidePos.GetPositionY(),
                                hidePos.GetPositionZ(), bot->GetMapId());
                    }
                    else
                        pos.Set(WS_FLAG_POS_ALLIANCE.GetPositionX(),
                                WS_FLAG_POS_ALLIANCE.GetPositionY(),
            ...
            else
            {
                ...
                if (supporter)
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (bot->GetTeamId() == TEAM_ALLIANCE)
                                pos.Set(WS_FLAG_POS_HORDE.GetPositionX(),
                                        WS_FLAG_POS_HORDE.GetPositionY(),
                                        WS_FLAG_POS_HORDE.GetPositionZ(), bot->GetMapId());
                            else
                                pos.Set(WS_FLAG_POS_ALLIANCE.GetPositionX(),
                                        WS_FLAG_POS_ALLIANCE.GetPositionY(),
            ...
            if (bot->GetTeamId() == TEAM_HORDE)  // HORDE
            {
                if (!BgObjective)
                {
                    ...
                    for (uint8 i = 0; i < 3; ++i)
                    {
                        ...
                        {
                            if (arathiBasinBG->GetCapturePointInfo(objective)._state ==
                            ...
                            {
                                if (GameObject *pGO =
                                        bg->GetBGObject(objective * BG_AB_OBJECTS_PER_NODE))
                                {
                                    float const distance = sqrt(bot->GetDistance(pGO));
                                    if (attackObjectiveDistance > distance)
                                    {
                               ...
                               // points. Afterwards pick random strategies
        {
            ...
            if (!bot->HasAura(BG_EY_NETHERSTORM_FLAG_SPELL))
            {
                ...
                else if (role < 8)  // Attack and Defend
                {
                    ...
                    {
                        if (eyeOfTheStormBG->GetCapturePointInfo(attackObjectivesFront[0])
                                    ._ownerTeamId != rootTeamIndex ||
                            eyeOfTheStormBG->GetCapturePointInfo(attackObjectivesFront[1])
                                    ._ownerTeamId != rootTeamIndex)
                        {
                        ...
                        else
                        {
                            ...
                            {
                                if (eyeOfTheStormBG->GetCapturePointInfo(attackObjectivesBack[0])
                                        ._ownerTeamId != rootTeamIndex)
                                {
                                    BgObjective = bg->GetBGObject(attackObjectivesBack[0]);
                                }
                                else if (eyeOfTheStormBG
                                             ->GetCapturePointInfo(attackObjectivesBack[1])
                                             ._ownerTeamId != rootTeamIndex)
                                {
                                    BgObjective = bg->GetBGObject(attackObjectivesBack[1]);
                                }
                        ...
                        if (eyeOfTheStormBG->GetCapturePointInfo(attackObjectivesFront[0])
                            ...
                            eyeOfTheStormBG->GetCapturePointInfo(attackObjectivesFront[1])
                                    ._ownerTeamId == rootTeamIndex &&
                            eyeOfTheStormBG->GetCapturePointInfo(attackObjectivesBack[0])
                                    ._ownerTeamId == rootTeamIndex &&
                            eyeOfTheStormBG->GetCapturePointInfo(attackObjectivesBack[1])
                                    ._ownerTeamId == rootTeamIndex)
                        {
                            role = urand(0, 9);
                        }
            ...
            if (bot->HasAura(BG_EY_NETHERSTORM_FLAG_SPELL))
            {
                ...
                if (eyeOfTheStormBG->GetCapturePointInfo(attackObjectivesFront[0])._ownerTeamId !=
                        rootTeamIndex &&
                    eyeOfTheStormBG->GetCapturePointInfo(attackObjectivesFront[1])._ownerTeamId !=
                        rootTeamIndex &&
                    eyeOfTheStormBG->GetCapturePointInfo(attackObjectivesBack[0])._ownerTeamId !=
                        rootTeamIndex &&
                    eyeOfTheStormBG->GetCapturePointInfo(attackObjectivesBack[1])._ownerTeamId !=
                        rootTeamIndex)
                {
                    // Retreat with flag
                ...
                else
                {
                    ...
                    // bot->GetGUID().ToString().c_str(), bot->GetTeamId() == TEAM_ALLIANCE ? "A" :
                    // "H", bot->GetLevel(), bot->GetName());
                    if (eyeOfTheStormBG->GetCapturePointInfo(EY_AttackObjectives[0])._ownerTeamId ==
                        rootTeamIndex)
                    {
                        areaTrigger = AT_FEL_REAVER_POINT;
                        flagDeliverPoint = EY_FLAG_RETURN_POS_REAVER_RUINS;
                    }
                    else if (eyeOfTheStormBG->GetCapturePointInfo(EY_AttackObjectives[1])
                                 ._ownerTeamId == rootTeamIndex)
                    {
                        areaTrigger = AT_BLOOD_ELF_POINT;
                        flagDeliverPoint = EY_FLAG_RETURN_POS_BLOOD_ELF_TOWER;
                    }
                    else if (eyeOfTheStormBG->GetCapturePointInfo(EY_AttackObjectives[2])
                                 ._ownerTeamId == rootTeamIndex)
                    {
                        areaTrigger = AT_DRAENEI_RUINS_POINT;
                        flagDeliverPoint = EY_FLAG_RETURN_POS_DRAENEI_RUINS;
                    }
                    else if (eyeOfTheStormBG->GetCapturePointInfo(EY_AttackObjectives[3])
                                 ._ownerTeamId == rootTeamIndex)
                    {
                    ...
                    if (bot->IsWithinDist3d(flagDeliverPoint.GetPositionX(),
                                            flagDeliverPoint.GetPositionY(),
                                            flagDeliverPoint.GetPositionZ(), INTERACTION_DISTANCE))
                    {
                ...
                if (!MoveTo(bot->GetMapId(), flagDeliverPoint.GetPositionX(),
                            flagDeliverPoint.GetPositionY(), flagDeliverPoint.GetPositionZ()))
                {
                    pos.Set(flagDeliverPoint.GetPositionX(), flagDeliverPoint.GetPositionY(),
                            flagDeliverPoint.GetPositionZ(), bot->GetMapId());
                    posMap["bg objective"] = pos;
                }
        ...
        case BATTLEGROUND_IC:
        {
            ...
            if (bot->GetTeamId() == TEAM_HORDE)  // HORDE
            {
                ...
                {
                    if (Creature *allyboss =
                            bg->GetBGCreature(BG_IC_NPC_HIGH_COMMANDER_HALFORD_WYRMBANE))
                    {
                ...
                if (!BgObjective)
                {
                    ...
                    if (role > 10)  // disabled
                    {
                        ...
                        if (firstTower < 2)
                        {
                            pos.Set(IC_CANNON_POS_HORDE1.GetPositionX(),
                                    IC_CANNON_POS_HORDE1.GetPositionY(),
                        ...
                        if (secondTower < 2)
                        {
                            pos.Set(IC_CANNON_POS_HORDE2.GetPositionX(),
                                    IC_CANNON_POS_HORDE2.GetPositionY(),
                    ...
                    {
                        // Capture Refinery
                        ICNodePoint const &nodePoint =
                            isleOfConquestBG->GetICNodePoint(NODE_TYPE_REFINERY);
                        if (nodePoint.nodeState != NODE_STATE_CONFLICT_H &&
                            nodePoint.nodeState != NODE_STATE_CONTROLLED_H)
                        {
                    ...
                    if (!BgObjective && role < 6 && urand(0, 1))  // Capture Docks
                    {
                        // Capture Docks
                        ICNodePoint const &nodePoint =
                            isleOfConquestBG->GetICNodePoint(NODE_TYPE_DOCKS);
                        if (nodePoint.nodeState != NODE_STATE_CONFLICT_H &&
                            nodePoint.nodeState != NODE_STATE_CONTROLLED_H)
                        {
                    ...
                    {
                        ICNodePoint const &nodePoint =
                            isleOfConquestBG->GetICNodePoint(NODE_TYPE_WORKSHOP);
                        if (nodePoint.nodeState != NODE_STATE_CONFLICT_H &&
                            nodePoint.nodeState != NODE_STATE_CONTROLLED_H)
                        {
                    ...
                    {
                        ICNodePoint const &nodePoint =
                            isleOfConquestBG->GetICNodePoint(NODE_TYPE_HANGAR);
                        if (nodePoint.nodeState != NODE_STATE_CONFLICT_H &&
                            nodePoint.nodeState != NODE_STATE_CONTROLLED_H)
                        {
                                ...
                                if (GameObject *gate = bg->GetBGObject(BG_IC_GO_ALLIANCE_GATE_3))
                                {
                                    if (controlsVehicle)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            pos.Set(506.782f + frand(-5, +5),
                                                    -828.594f + frand(-5, +5), 24.313f,
                                                    bot->GetMapId());
                                            posMap["bg objective"] = pos;

                                            // set siege position
                                            PositionInfo siegePos =
                                                context->GetValue<PositionMap &>("position")
                                                    ->Get()["bg siege"];
                                    ...
                                    {
                                        pos.Set(506.782f + frand(-5, +5), -828.594f + frand(-5, +5),
                                                24.313f, bot->GetMapId());
                                        posMap["bg objective"] = pos;
                                        return true;
                                        // BgObjective = gate;
                                    }
                    ...
                    if (!BgObjective && gateOpen)  // Capture Keep
                    {
                        ...
                        ICNodePoint const &nodePoint =
                            isleOfConquestBG->GetICNodePoint(NODE_TYPE_GRAVEYARD_A);
                        if (nodePoint.nodeState != NODE_STATE_CONFLICT_H &&
                            nodePoint.nodeState != NODE_STATE_CONTROLLED_H)
                        {
            ...
            if (bot->GetTeamId() == TEAM_ALLIANCE)  // ALLIANCE
            {
                ...
                if (!BgObjective)
                {
                    ...
                    if (role > 10)  // disabled
                    {
                        ...
                        if (firstTower < 3)
                        {
                            pos.Set(IC_CANNON_POS_ALLIANCE1.GetPositionX(),
                                    IC_CANNON_POS_ALLIANCE1.GetPositionY(),
                        ...
                        if (secondTower < 3)
                        {
                            pos.Set(IC_CANNON_POS_ALLIANCE2.GetPositionX(),
                                    IC_CANNON_POS_ALLIANCE2.GetPositionY(),
                    ...
                    if (role < 3)  // Capture Side base or Docks
                    {
                        // Capture Quarry
                        ICNodePoint const &nodePoint =
                            isleOfConquestBG->GetICNodePoint(NODE_TYPE_QUARRY);
                        if (nodePoint.nodeState != NODE_STATE_CONFLICT_A &&
                            nodePoint.nodeState != NODE_STATE_CONTROLLED_A)
                        {
                            ...
                            else
                            {
                                if (GameObject *gate = bg->GetBGObject(BG_IC_GO_HORDE_GATE_3))
                                {
                                    if (controlsVehicle)
                                    {
                                        ...
                                        {
                                            // take a siege position
                                            pos.Set(1091.273f + frand(-5, +5),
                                                    -763.619f + frand(-5, +5), 42.352f,
                                                    bot->GetMapId());
                                            posMap["bg objective"] = pos;
                    ...
                    if (!BgObjective && gateOpen)  // Capture Keep
                    {
                        ...
                        ICNodePoint const &nodePoint =
                            isleOfConquestBG->GetICNodePoint(NODE_TYPE_GRAVEYARD_H);
                        if (nodePoint.nodeState != NODE_STATE_CONFLICT_A &&
                            nodePoint.nodeState != NODE_STATE_CONTROLLED_A)
                        {
            ...
            {
                pos.Set(BgObjective->GetPositionX(), BgObjective->GetPositionY(),
                        BgObjective->GetPositionZ(), bot->GetMapId());
                posMap["bg objective"] = pos;
                return true;
            }
            break;
...
bool BGTactics::moveToObjective()
{
    ...
    {
        Position const caveSpawn =
            bot->GetTeamId() == TEAM_ALLIANCE ? AV_CAVE_SPAWN_ALLIANCE : AV_CAVE_SPAWN_HORDE;
...
bool BGTactics::selectObjectiveWp(std::vector<BattleBotPath *> const &vPaths)
{
    ...
    float botDistanceScoreSubtract =
        8.0f;  // path score modifier - lower = less likely to chose a further path (it's basically
               // the distance from bot that's ignored)
    float botDistanceScoreMultiply =
        3.0f;  // path score modifier - higher = less likely to chose a further path (it's basically
    ...
    for (auto const &path : vPaths)
    {
        ...
        if (bot->GetTeamId() == TEAM_ALLIANCE &&
            std::find(vPaths_AllyMine.begin(), vPaths_AllyMine.end(), path) !=
                vPaths_AllyMine.end())
            continue;

        if (bot->GetTeamId() == TEAM_HORDE &&
            std::find(vPaths_HordeMine.begin(), vPaths_HordeMine.end(), path) !=
                vPaths_HordeMine.end())
            continue;

        BattleBotWaypoint &startPoint = ((*path)[0]);
        float const startPointDistToDestination =
            sqrt(Position(pos.x, pos.y, pos.z, 0.f)
                     .GetExactDist(startPoint.x, startPoint.y, startPoint.z));
        BattleBotWaypoint &endPoint = ((*path)[path->size() - 1]);
        float const endPointDistToDestination = sqrt(
            Position(pos.x, pos.y, pos.z, 0.f).GetExactDist(endPoint.x, endPoint.y, endPoint.z));

        bool reverse = startPointDistToDestination < endPointDistToDestination;

        // dont travel reverse if it's a reverse paths
        if (reverse && std::find(vPaths_NoReverseAllowed.begin(), vPaths_NoReverseAllowed.end(),
                                 path) != vPaths_NoReverseAllowed.end())
            continue;
        ...
        if (closestPointIndex == (reverse ? 0 : path->size() - 1) ||
        ...
        float pathScore = (closestPointDistToBot < botDistanceScoreSubtract
                               ? 0.0f
                               : ((closestPointDistToBot - botDistanceScoreSubtract) *
                                  botDistanceScoreMultiply)) +
                          distToDestination;
...
bool BGTactics::moveToObjectiveWp(BattleBotPath *const &currentPath, uint32 currentPoint,
                                  bool reverse)
{
    if (!currentPath)
        return false;
    ...
    return MoveTo(bot->GetMapId(), nextPoint.x + frand(-2, 2), nextPoint.y + frand(-2, 2),
                  nextPoint.z);
}
...
bool BGTactics::startNewPathBegin(std::vector<BattleBotPath *> const &vPaths)
{
    ...
    for (auto const &pPath : vPaths)
    {
        // skip mine paths of own faction
        if (bot->GetTeamId() == TEAM_ALLIANCE &&
            std::find(vPaths_AllyMine.begin(), vPaths_AllyMine.end(), pPath) !=
                vPaths_AllyMine.end())
            continue;

        if (bot->GetTeamId() == TEAM_HORDE &&
            std::find(vPaths_HordeMine.begin(), vPaths_HordeMine.end(), pPath) !=
                vPaths_HordeMine.end())
            continue;
...
bool BGTactics::startNewPathFree(std::vector<BattleBotPath *> const &vPaths)
{
    ...
    for (auto const &pPath : vPaths)
    {
        ...
        for (uint32 i = 0; i < pPath->size(); i++)
        {
            ...
            float const distanceToPoint =
                sqrt(bot->GetDistance(waypoint.x, waypoint.y, waypoint.z));
...
bool BGTactics::atFlag(std::vector<BattleBotPath *> const &vPaths,
                       std::vector<uint32> const &vFlagIds)
{
    Battleground *bg = bot->GetBattleground();
    if (!bg)
    ...
    if (!closePlayers.empty())
    {
        for (auto &guid : closePlayers)
        {
            ...
            if (pFriend->GetCurrentSpell(CURRENT_GENERIC_SPELL) &&
                pFriend->GetCurrentSpell(CURRENT_GENERIC_SPELL)->m_spellInfo->Id ==
                    SPELL_CAPTURE_BANNER)
            {
                resetObjective();
    ...
    for (ObjectGuid const guid : closeObjects)
    {
        ...
        std::vector<uint32>::const_iterator f =
            find(vFlagIds.begin(), vFlagIds.end(), go->GetEntry());
        if (f == vFlagIds.end())
            continue;
        ...
        switch (bgType)
        {
            ...
            case BATTLEGROUND_WS:
            {
                ...
                else
                {
                    ...
                    // bot->Say(out.str(), LANG_UNIVERSAL);
                    return MoveTo(bot->GetMapId(), go->GetPositionX(), go->GetPositionY(),
                                  go->GetPositionZ());
                }
                break;
            }
...
bool BGTactics::useBuff()
{
    ...
    bool needSpeed =
        (bgType != BATTLEGROUND_WS || bot->HasAura(BG_WS_SPELL_WARSONG_FLAG) ||
    ...
    for (ObjectGuid const guid : closeObjects)
    {
        ...
        if (foundBuff)
        {
            ...
            return MoveTo(go->GetMapId(), go->GetPositionX(), go->GetPositionY(),
                          go->GetPositionZ());
        }
    }
...
uint32 BGTactics::getDefendersCount(Position point, float range, bool combat)
{
    ...
    for (auto &guid : bg->GetBgMap()->GetPlayers())
    {
        ...
        if (player->IsAlive() && player->GetTeamId() == bot->GetTeamId())
        {
            ...
            if (sServerFacade->GetDistance2d(player, point.GetPositionX(), point.GetPositionY()) <
                range)
                ++defCount;
        }
    }
...
bool BGTactics::IsLockedInsideKeep()
{
    ...
    bool isInside = false;
    if (bot->GetTeamId() == TEAM_ALLIANCE && bot->GetPositionX() < 410.0f &&
        bot->GetPositionY() > -900.0f && bot->GetPositionY() < -765.0f)
        isInside = true;
    if (bot->GetTeamId() == TEAM_HORDE && bot->GetPositionX() > 1153.0f &&
        bot->GetPositionY() > -849.0f && bot->GetPositionY() < -679.0f)
        isInside = true;
    ...
    if (closestPortal)
    {
        ...
        {
            return MoveTo(bot->GetMapId(), closestPortal->GetPositionX(),
                          closestPortal->GetPositionY(), closestPortal->GetPositionZ());
        }
    }
...
bool ArenaTactics::moveToCenter(Battleground *bg)
{
    ...
    switch (bg->GetBgTypeID())
    {
        case BATTLEGROUND_BE:
            if (Preference > 10)
                MoveTo(bg->GetMapId(), 6185.0f + frand(-2, +2), 236.0f + frand(-2, +2), 6.0f, false,
                       true);
            else
                MoveTo(bg->GetMapId(), 6240.0f + frand(-2, +2), 262.0f + frand(-2, +2), 2.0f, false,
                       true);
            break;
        case BATTLEGROUND_RL:
            if (Preference < 5)
                MoveTo(bg->GetMapId(), 1320.0f + frand(-2, +2), 1672.0f + frand(-2, +2), 38.0f,
                       false, true);
            else
                MoveTo(bg->GetMapId(), 1273.0f + frand(-2, +2), 1666.0f + frand(-2, +2), 36.0f,
                       false, true);
            break;
        case BATTLEGROUND_NA:
            MoveTo(bg->GetMapId(), 4055.0f + frand(-5, +5), 2921.0f + frand(-5, +5), 15.1f, false,
                   true);
            break;
        case BATTLEGROUND_DS:
            if (!MoveTo(bg->GetMapId(), 1291.58f + frand(-5, +5), 790.87f + frand(-5, +5), 7.8f,
                        false, true))
            {
        ...
        case BATTLEGROUND_RV:
            MoveTo(bg->GetMapId(), 764.65f + frand(-2, +2), -283.85f + frand(-2, +2), 28.28f, false,
                   true);
            break;
        default:
            break;
