...
bool StoreLootAction::AuctionItem(uint32 itemId)
{
    ...
    uint32 price = oldItem->GetCount() * proto->BuyPrice *
sRandomPlayerbotMgr->GetBuyMultiplier(bot);
...
uint32 stackCount = urand(1, proto->GetMaxStackSize());
    ...
    uint32 auction_time = uint32(urand(8, 24) * HOUR *
sWorld->getConfig(CONFIG_FLOAT_RATE_AUCTION_TIME));
    ...
    LOG_ERROR("playerbots", "AhBot {} added {} of {} to auction {} for {}..{}",
bot->GetName().c_str(), stackCount, proto->Name1.c_str(), 1, bidPrice, buyoutPrice);
...
bool StoreLootAction::Execute(Event event)
{
    ...
    for (uint8 i = 0; i < items; ++i)
    {
        ...
        // LOG_ERROR("playerbots", "Bot {} is looting {} {} for usage {}.", bot->GetName().c_str(),
        // itemcount, proto->Name1.c_str(), usage);
    }
...
bool StoreLootAction::IsLootAllowed(uint32 itemid, PlayerbotAI* botAI)
{
    ...
    // canLoot =
    // sPlayerbotAIConfig->IsInRandomAccountList(botAI->GetBot()->GetSession()->GetAccountId());
