...
{
    bool noattackers = AI_VALUE2(bool, "combat", "self target")
                           ? (AI_VALUE(uint8, "attacker count") > 0 ? false : true)
                           : true;
    ...
    bool dps = (AI_VALUE(Unit *, "dps target") ||
                (!bot->InBattleground() && AI_VALUE(Unit *, "grind target")));
    bool fartarget = (enemy && sServerFacade->IsDistanceGreaterThan(
                                   AI_VALUE2(float, "distance", "enemy player target"), 40.0f)) ||
                     (dps && sServerFacade->IsDistanceGreaterThan(
                                 AI_VALUE2(float, "distance", "dps target"), 50.0f));
    ...
    {
        attackdistance =
            (enemy || dps) && sServerFacade->IsDistanceLessThan(
                                  AI_VALUE2(float, "distance", "current target"), attack_distance);
        chasedistance = enemy &&
                        sServerFacade->IsDistanceGreaterThan(
                            AI_VALUE2(float, "distance", "enemy player target"), 45.0f) &&
    ...
    if (master != nullptr && !bot->InBattleground())
    {
        ...
        // bool farFromMaster = sServerFacade->GetDistance2d(bot, master) >
        ...
        // if (!bot->IsFlying() && ((!farFromMaster && !master->IsMounted()) || attackdistance) &&
        // bot->IsMounted())
        // {
        //     WorldPacket emptyPacket;
        //     bot->GetSession()->HandleCancelMountAuraOpcode(emptyPacket);
    ...
    if (bot->InBattleground() && !attackdistance && (noattackers || fartarget) &&
        !bot->IsInCombat() && !bot->IsMounted())
    {
    ...
    //     if (((!AI_VALUE(GuidVector, "possible rpg targets").empty()) && noattackers && !dps &&
    //     !enemy) && urand(0, 100) > 50)
    //         return Mount();
    // }
    ...
    if (!bot->IsFlying() && attackdistance && bot->IsMounted() &&
...
bool CheckMountStateAction::Mount()
{
    ...
    if (master && !master->GetAuraEffectsByType(SPELL_AURA_MOUNTED).empty() &&
        !bot->InBattleground())
    {
        masterSpell = master->GetAuraEffectsByType(SPELL_AURA_MOUNTED).front()->GetSpellInfo();
        masterSpeed =
            std::max(masterSpell->Effects[1].BasePoints, masterSpell->Effects[2].BasePoints);
    }
    else
    {
        masterSpeed = 59;
             ...
             itr != bot->GetSpellMap().end(); ++itr)
        {
            ...
            int32 effect =
                std::max(spellInfo->Effects[1].BasePoints, spellInfo->Effects[2].BasePoints);
         ...
         ++itr)
    {
        ...
        if (index == 0 && std::max(spellInfo->Effects[EFFECT_1].BasePoints,
                                   spellInfo->Effects[EFFECT_2].BasePoints) > 59)
            hasSwiftMount = true;

        if (index == 1 && std::max(spellInfo->Effects[EFFECT_1].BasePoints,
                                   spellInfo->Effects[EFFECT_2].BasePoints) > 149)
            hasSwiftMount = true;
    ...
    {
        masterMountType =
            (masterSpell->Effects[1].ApplyAuraName ==
        ...
        for (auto i : spells)
        {
            ...
            for (auto itr : ids)
            {
                ...
                if (masterMountType == 0 && masterSpeed > 59 &&
                    std::max(spellInfo->Effects[EFFECT_1].BasePoints,
                             spellInfo->Effects[EFFECT_2].BasePoints) < 99)
                    spells[59].clear();

                if (masterMountType == 1 && masterSpeed > 149 &&
                    std::max(spellInfo->Effects[EFFECT_1].BasePoints,
                             spellInfo->Effects[EFFECT_2].BasePoints) < 279)
                    spells[149].clear();
            }
        }
    }
