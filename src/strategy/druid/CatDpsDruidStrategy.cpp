...
CatDpsDruidStrategy::CatDpsDruidStrategy(PlayerbotAI *botAI) : FeralDruidStrategy(botAI) { actionNodeFactories.Add(new CatDpsDruidStrategyActionNodeFactory()); }

NextAction **CatDpsDruidStrategy::getDefaultActions() { return NextAction::array(0, new NextAction("mangle (cat)", ACTION_DEFAULT + 0.1f), nullptr); }
...
void CatDpsDruidStrategy::InitTriggers(std::vector<TriggerNode *> &triggers)
{
    ...
    // triggers.push_back(new TriggerNode("player has no flag", NextAction::array(0, new
    // NextAction("prowl", ACTION_HIGH), nullptr))); triggers.push_back(new TriggerNode("enemy out
    // of melee", NextAction::array(0, new NextAction("prowl", ACTION_INTERRUPT + 1), nullptr)));
...
void CatAoeDruidStrategy::InitTriggers(std::vector<TriggerNode *> &triggers) { triggers.push_back(new TriggerNode("medium aoe", NextAction::array(0, new NextAction("swipe (cat)", ACTION_HIGH + 2), nullptr))); }
