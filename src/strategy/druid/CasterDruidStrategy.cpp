...
{
    return NextAction::array(0, new NextAction("starfall", ACTION_DEFAULT + 0.2f), new NextAction("wrath", ACTION_DEFAULT + 0.1f),
...
void CasterDruidAoeStrategy::InitTriggers(std::vector<TriggerNode *> &triggers)
{
    ...
    triggers.push_back(new TriggerNode("light aoe", NextAction::array(0, new NextAction("starfall", ACTION_NORMAL + 5), new NextAction("insect swarm on attacker", ACTION_NORMAL + 3), new NextAction("moonfire on attacker", ACTION_NORMAL + 3), NULL)));
}

void CasterDruidDebuffStrategy::InitTriggers(std::vector<TriggerNode *> &triggers) { triggers.push_back(new TriggerNode("faerie fire", NextAction::array(0, new NextAction("faerie fire", ACTION_HIGH), nullptr))); }
