...
void GenericHunterStrategy::InitTriggers(std::vector<TriggerNode *> &triggers)
{
    ...
    triggers.push_back(
        new TriggerNode("enemy too close for auto shot",
    ...
    triggers.push_back(
        new TriggerNode("tranquilizing shot",
...
{
    triggers.push_back(new TriggerNode(
        "rapid fire", NextAction::array(0, new NextAction("rapid fire", 16.0f), nullptr)));
}
...
void HunterCcStrategy::InitTriggers(std::vector<TriggerNode *> &triggers)
{
    ...
    triggers.push_back(new TriggerNode(
        "freezing trap",
        NextAction::array(0, new NextAction("freezing trap on cc", ACTION_HIGH + 3), nullptr)));
}
