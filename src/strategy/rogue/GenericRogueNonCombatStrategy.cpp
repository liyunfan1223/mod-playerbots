...
{
   public:
    GenericRogueNonCombatStrategyActionNodeFactory() { creators["use deadly poison on off hand"] = &use_deadly_poison_on_off_hand; }
...
void GenericRogueNonCombatStrategy::InitTriggers(std::vector<TriggerNode *> &triggers)
{
    ...
    // triggers.push_back(new TriggerNode("unstealth", NextAction::array(0, new
    // NextAction("unstealth", 1.0f), nullptr))); triggers.push_back(new TriggerNode("often",
    // NextAction::array(0, new NextAction("apply poison", 1.0f), nullptr)));

    triggers.push_back(new TriggerNode("main hand weapon no enchant", NextAction::array(0, new NextAction("use instant poison on main hand", 20.0f), NULL)));

    triggers.push_back(new TriggerNode("off hand weapon no enchant", NextAction::array(0, new NextAction("use deadly poison on off hand", 19.0f), NULL)));
