...
bool WithinAreaTrigger::IsPointInAreaTriggerZone(AreaTrigger const *atEntry, uint32 mapid, float x, float y, float z, float delta)
{
    ...
    else
    {
        ...
        // rotate the players position instead of rotating the whole cube, that way we can make a
        // simplified is-in-cube check and we have to calculate only one point instead of 4
