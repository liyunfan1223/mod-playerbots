...
bool EnemyTooCloseForSpellTrigger::IsActive()
{
    ...
    return target && (target->GetVictim() != bot || target->isFrozen() || target->HasRootAura() || !target->CanFreeMove()) &&
...
bool EnemyTooCloseForAutoShotTrigger::IsActive()
{
    ...
    return !trapToCast && (target->GetVictim() != bot || target->isFrozen() || target->HasRootAura() || !target->CanFreeMove()) &&
