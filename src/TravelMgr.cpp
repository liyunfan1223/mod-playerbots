/*
 * Copyright (C) 2016+ AzerothCore <www.azerothcore.org>, released under GNU GPL
 * v2 license, you may redistribute it and/or modify it under version 2 of the
...
WorldPosition WorldPosition::lastInRange(std::vector<WorldPosition> list, float minDist, float maxDist)
{
    ...
    for (auto& p : list)
    {
        ...
        if (maxDist > 0 && totalDist > maxDist)
            continue;  // We do not break here because the path may loop back and have
                       // a second startDist point.
            ...
            if (!sTravelMgr->isBadVmap(mapId, x, y))
            {
                ...
                switch (vmapLoadResult)
                {
                    case VMAP::VMAP_LOAD_RESULT_OK:
                        // LOG_ERROR("playerbots", "VMAP loaded name:{}, id:{}, x:{}, y:{}
                        // (vmap rep.: x:{}, y:{})", mapName, mapId, x, y, x, y);
                        break;
                    case VMAP::VMAP_LOAD_RESULT_ERROR:
                        // LOG_ERROR("playerbots", "Could not load VMAP name:{}, id:{}, x:{},
                        // y:{} (vmap rep.: x:{}, y:{})", mapName, mapId, x, y, x, y);
                        sTravelMgr->addBadVmap(mapId, x, y);
                        break;
                    case VMAP::VMAP_LOAD_RESULT_IGNORED:
                        sTravelMgr->addBadVmap(mapId, x, y);
                        // LOG_INFO("playerbots", "Ignored VMAP name:{}, id:{}, x:{}, y:{}
                        // (vmap rep.: x:{}, y:{})", mapName, mapId, x, y, x, y);
                        break;
                }
...
// A single pathfinding attempt from one position to another. Returns
// pathfinding status and path.
...
// A sequential series of pathfinding attempts. Returns the complete path and if
// the patfinder eventually found a way to the destination.
                                                          ...
                                                          uint8 maxAttempt)
{
    ...
    for (uint32 i = 0; i < maxAttempt; i++)
    {
        ...
        // Append the path excluding the start (this should be the same as the end
        // of the startPath)
...
bool RpgTravelDestination::isActive(Player* bot)
{
    ...
    // Once the target rpged with it is added to the ignore list. We can now move
    // on.
...
void TravelMgr::logQuestError(uint32 errorNr, Quest* quest, uint32 objective, uint32 unitId, uint32 itemId)
{
    ...
    else if (errorNr == 2)
    {
        LOG_ERROR("playerbots",
                  "Quest {} [{}] needs {} [{}] for objective {} but none is found "
                  "in the world.",
    ...
    {
        LOG_ERROR("playerbots",
                  "Quest {} [{}] needs {} [{}] for loot of item {} [{}] for "
                  "objective {} but none is found in the world.",
                  quest->GetTitle().c_str(), quest->GetQuestId(), unitName.c_str(), unitId, proto->Name1.c_str(),
                  itemId, objective);
    }
    else if (errorNr == 5)
    {
        LOG_ERROR("playerbots",
                  "Quest {} [{}] needs item {} [{}] for objective {} but none is "
                  "found in the world.",
...
void TravelMgr::LoadQuestTravelTable()
{
    ...
    for (auto cQuest : cQuestMap)
    {
        ...
        for (auto cObj : cQuest.second)
        {
            ...
            {
                LOG_INFO("playerbots", " {} {}", cCre.GetTypeName().c_str(),
    cCre.GetEntry());
            }
        }
    }
    */
    ...
    /*
    //                          0    1  2   3          4          5          6 7 8
    std::string const query = "SELECT
    0,guid,id,map,position_x,position_y,position_z,orientation, (SELECT COUNT(*)
    FROM creature k WHERE c.id1 = k.id1) FROM creature c UNION ALL SELECT
    1,guid,id,map,position_x,position_y,position_z,orientation, (SELECT COUNT(*)
    FROM gameobject h WHERE h.id = g.id)  FROM gameobject g";
    ...
    query = "SELECT 0, 0, id, quest FROM creature_queststarter UNION ALL SELECT 0,
    1, id, quest FROM creature_questender UNION ALL SELECT 1, 0, id, quest FROM
    gameobject_queststarter UNION ALL SELECT 1, 1, id, quest FROM
    ...
    query = "SELECT 0, ct.entry, item FROM creature_template ct JOIN
    creature_loot_template clt ON (ct.lootid = clt.entry) UNION ALL SELECT 0,
    entry, item FROM npc_vendor UNION ALL SELECT 1, gt.entry, item FROM
    gameobject_template gt JOIN gameobject_loot_template glt ON (gt.TYPE = 3 AND
    ...
    */
    ...
    sObjectMgr->GetItemTemplate(quest->RequiredItemId[i]); if (!proto)
                {
    ...
    sPlayerbotAIConfig->tooCloseDistance, sPlayerbotAIConfig->sightDistance,
    l.item); loc->setExpireDelay(1 * 60 * 1000); loc->setMaxVisitors(100, 1);
                    ...
                    if (loc->getPoints(0).empty())
                    {
                        logQuestError(4, quest, i, entry,
    quest->RequiredItemId[i]); delete loc; continue;
                    }
            ...
            if (!container->questGivers.empty() || !container->questTakers.empty()
    || hasError)
            {
                quests.insert(std::make_pair(questId, container));
   ...
   if (!fullNavPointReload && true)
    ...
    bool reloadNavigationPoints = false || fullNavPointReload ||
   storeNavPointReload;
   ...
   sObjectMgr->GetCreatureTemplate(u.entry); if (!cInfo) continue;
   ...
   allowedNpcFlags.end(); ++i)
        ...
        for (uint32 i = 0; i < sTaxiPathStore.GetNumRows(); ++i)
        {
            ...
            TaxiNodesEntry const* startTaxiNode =
   sTaxiNodesStore.LookupEntry(taxiPath->from); if (!startTaxiNode) continue;

            TaxiNodesEntry const* endTaxiNode =
   sTaxiNodesStore.LookupEntry(taxiPath->to); if (!endTaxiNode) continue;

            TaxiPathNodeList const& nodes = sTaxiPathNodesByPath[taxiPath->ID];
            if (nodes.empty())
                continue;

            WorldPosition startPos(startTaxiNode->map_id, startTaxiNode->x,
   startTaxiNode->y, startTaxiNode->z); WorldPosition endPos(endTaxiNode->map_id,
   endTaxiNode->x, endTaxiNode->y, endTaxiNode->z);

            TravelNode* startNode = sTravelNodeMap->getNode(&startPos,
   nullptr, 15.0f); TravelNode* endNode = sTravelNodeMap->getNode(&endPos,
   nullptr, 15.0f);
   ...
   TravelNodePathType::flightPath, i, true); travelPath.setPath(ppath);
   ...
   sObjectMgr->GetCreatureTemplate(u.entry); if (!cInfo) continue;
            ...
            if (cInfo->rank == 3 || (cInfo->rank == 1 && !pos.isOverworld() && u.c
   == 1))
            {
                std::string const nodeName = cInfo->Name;
                sTravelNodeMap->addNode(&pos, nodeName, true, true);
        ...
        for (uint32 i = 0; i < MAX_RACES; i++)
        {
            for (uint32 j = 0; j < MAX_CLASSES; j++)
            {
                ...
                pos = WorldPosition(info->mapId, info->positionX, info->positionY,
   info->positionZ, info->orientation);
        ...
        //Transports
        GameObjectTemplateContainer const* goTemplates =
   sObjectMgr->GetGameObjectTemplates(); for (auto const& iter : *goTemplates)
        {
                ...
                if (path.empty())
                {
                    if (animation)
                    {
                        ...
                        for (auto& u : units)
                        {
                            ...
                            for (auto& p : aPath)
                            {
                                float dx = cos(u.o) * p.second->X - sin(u.o) *
   p.second->Y; float dy = sin(u.o) * p.second->X + cos(u.o) * p.second->Y;
                                WorldPosition pos = WorldPosition(u.map, u.x + dx,
   u.y + dy, u.z + p.second->Z, u.o);
                                ...
                                {
                                    TravelNode* node =
   sTravelNodeMap->addNode(&pos, data->name, true, true, true, iter.first);
                                    ...
                                    {
                                        float totalTime = (p.second->TimeSeg -
   timeStart) / 1000.0f; TravelNodePath travelPath(0.1f, totalTime, (uint8)
   TravelNodePathType::transport, entry, true); node->setPathTo(prevNode,
   travelPath); ppath.clear(); ppath.push_back(pos); timeStart =
   p.second->TimeSeg;
                                    }
                            ...
                            if (prevNode)
                            {
                                for (auto& p : aPath)
                                {
                                    float dx = cos(u.o) * p.second->X - sin(u.o) *
   p.second->Y; float dy = sin(u.o) * p.second->X + cos(u.o) * p.second->Y;
                                    WorldPosition pos = WorldPosition(u.map, u.x +
   dx, u.y + dy, u.z + p.second->Z, u.o);
                                    ...
                                    {
                                        TravelNode* node =
   sTravelNodeMap->addNode(&pos, data->name, true, true, true, iter.first); if
   (node != prevNode)
                                        {
                                            float totalTime = (p.second->TimeSeg -
   timeStart) / 1000.0f; TravelNodePath travelPath(0.1f, totalTime, (uint8)
   TravelNodePathType::transport, entry, true); travelPath.setPath(ppath);
                ...
                else //Boats/Zepelins
                {
                    ...
                    for (auto& p : path)
                    {
                        WorldPosition pos = WorldPosition(p->mapid, p->x, p->y,
   p->z, 0);
                       ...
                       //     pos.setZ(pos.getZ() - 17.0f);
                        ...
                        if (p->delay > 0)
                        {
                            TravelNode* node = sTravelNodeMap->addNode(&pos,
   data->name, true, true, true, iter.first);
   ...
   TravelNodePathType::transport, entry, true); travelPath.setPathAndCost(ppath,
   moveSpeed); node->setPathTo(prevNode, travelPath); ppath.clear();
                                ppath.push_back(pos);
                            }
                    ...
                    if (prevNode)
                    {
                        ...
                        for (auto& p : path)
                        {
                            WorldPosition pos = WorldPosition(p->mapid, p->x,
   p->y, p->z, 0);
                            ...
                            if (p->delay > 0)
                            {
                                TravelNode* node = sTravelNodeMap->getNode(&pos,
   nullptr, 5.0f); if (node != prevNode)
                                {
                                    TravelNodePath travelPath(0.1f, 0.0, (uint8)
   TravelNodePathType::transport, entry, true); travelPath.setPathAndCost(ppath,
   moveSpeed); node->setPathTo(prevNode, travelPath);
                                }
                            }
                        }
                    }
        ...
        for (auto& loc : exploreLocs)
        {
            ...
            TravelNode* node = sTravelNodeMap->addNode(&pos, pos.getAreaName(),
   true, true, false);
        }
    ...
    */
    ...
    /*
    bool preloadNodePaths = false || fullNavPointReload || storeNavPointReload;
    //Calculate paths using PathGenerator. bool preloadReLinkFullyLinked = false
    || fullNavPointReload || storeNavPointReload;      //Retry nodes that are
    fully linked. bool preloadUnlinkedPaths = false || fullNavPointReload; //Try
    to connect points currently unlinked. bool preloadWorldPaths = true; //Try to
    load paths in overworld. bool preloadInstancePaths = true;         //Try to
    load paths in instances. bool preloadSubPrint = false;             //Print
    output every 2%.
    ...
    if (preloadNodePaths)
    {
        ...
        for (auto& startNode : sTravelNodeMap->getNodes())
        {
            ...
            for (auto& endNode : sTravelNodeMap->getNodes())
            {
                ...
                if (!startNode->getPosition()->isOverworld() &&
    !preloadInstancePaths) continue;
    ...
    !startNode->getPathTo(endNode)->getComplete())
        ...
        LOG_INFO("playerbots", ">> Loaded paths for {} nodes.",
    sTravelNodeMap->getNodes().size());
    }
    ...
    if (removeLowLinkNodes)
    {
        ...
        for (auto& node : sTravelNodeMap->getNodes())
        {
            ...
            if (std::find(goodNodes.begin(), goodNodes.end(), node) !=
    goodNodes.end()) continue;

            if (std::find(remNodes.begin(), remNodes.end(), node) !=
    remNodes.end()) continue;
        ...
        LOG_INFO("playerbots", ">> Checked {} nodes.",
    sTravelNodeMap->getNodes().size());
    }
    ...
    if (cleanUpNodeLinks)
    {
        ...
        LOG_INFO("playerbots", ">> Cleaned paths for {} nodes.",
    sTravelNodeMap->getNodes().size());
    }
...
#define my_sizeof(type) ((char*)(&type + 1) - (char*)(&type))
        ...
        static float const middleToSouthLimit[] = {-2402.010000f, 4255.7000000f, -2475.933105f,
                                                   3199.5683590f,  // Desolace
                                                   -2344.124023f, 1756.1643070f, -2826.438965f,
                                                   403.82473800f,                 // Mulgore
                                                   -3472.819580f, 182.52247600f,  // Feralas
                                                   -4365.006836f, -1602.575439f,  // the Barrens
                                                   -4515.219727f, -1681.356079f, -4543.093750f,
                                                   -1882.869385f,  // Thousand Needles
                                                   -4824.160000f, -2310.110000f, -5102.913574f,
                                                   -2647.062744f, -5248.286621f, -3034.536377f,
                                                   -5246.920898f, -3339.139893f, -5459.449707f,
                                                   -4920.155273f,  // Tanaris
    ...
    for (auto& node : sTravelNodeMap->getNodes())
    {
        ...
        for (uint32 x = 0; x < 2000; x++)
        {
            for (uint32 y = 0; y < 2000; y++)
            {
                ...
                //pos->getMap()->GetHitPosition(nx, ny, nz + 200.0f, nx, ny, nz,
    -0.5f);

                if (!pos->getMap()->GetHeightInRange(nx, ny, nz, 5000.0f)) //
    GetHeight can fail continue;

                WorldPosition  npos = WorldPosition(pos->getMapId(), nx, ny, nz,
    0.0); uint32 area = path.getArea(npos.getMapId(), npos.getX(), npos.getY(),
    npos.getZ());

                std::ostringstream out;
                out << std::fixed << area << "," << npos.getDisplayX() << "," <<
    npos.getDisplayY(); sPlayerbotAIConfig->log(7, out.str().c_str());
            }
        }
        k++;
    ...
    for (auto i : exploreLocs)
    {
        for (auto j : i.second->getPoints())
        {
            ...
            name.erase(remove(name.begin(), name.end(), '\"'), name.end());
            out << std::fixed << std::setprecision(2) << name.c_str() << "," <<
    i.first << "," << j->getDisplayX() << "," << j->getDisplayY() << "," <<
    j->getX() << "," << j->getY() << "," << j->getZ(); sPlayerbotAIConfig->log(5,
    out.str().c_str());
        }
    }
...
uint32 TravelMgr::getDialogStatus(Player* pPlayer, int32 questgiver, Quest const* pQuest)
{
    ...
    for (QuestRelations::const_iterator itr = rbounds.first; itr != rbounds.second; ++itr)
    {
        ...
        if (status == QUEST_STATUS_NONE)  // For all other cases the mark is handled
                                          // either at some place else, or with
...
// Selects a random WorldPosition from a list. Use a distance weighted
// distribution.
                                                    ...
                                                    uint32 amount)
{
    ...
    // List of weights based on distance (Gausian curve that starts at 100 and
    // lower to 1 at 1000 distance) std::transform(retVec.begin(), retVec.end(),
    // std::back_inserter(weights), [center](WorldPosition point) { return 1 +
    // 1000 * exp(-1 * pow(point.distance(center) / 400.0, 2)); });

    // List of weights based on distance (Twice the distance = half the weight).
    // Caps out at 200.0000 range.
    ...
    for (uint32 nr = 0; nr < amount; nr++)
    {
        ...
        for (unsigned i = 0; i < points.size(); ++i)
            if (rnd < weights[i] && (retVec.empty() || std::find(retVec.begin(),
    retVec.end(), points[i]) == retVec.end()))
            {
                                                   ...
                                                   uint32 amount)
{
    ...
    // Peiru: Crash failsafe - if the retVec is still empty but points exist,
    // return first point
