...
void PerformanceMonitor::PrintStats(bool perTick, bool fullStack)
{
    ...
    if (!perTick)
    {
        ...
        LOG_INFO("playerbots",
                 ...
                 "BOT]------------------------------------------------------");
        LOG_INFO("playerbots",
                 "percentage     time  |     min ..     max (      "
                 "avg  of      count) - type      : name");
        LOG_INFO("playerbots",
                 "-----------------------------------------------------------------"
                 "--------------------------------------");
             ...
             i != data.end(); ++i)
        {
            ...
            for (auto& name : names)
            {
                ...
                {
                    LOG_INFO("playerbots",
                             "{:7.3f}% {:10.3f}s | {:7.1f} .. {:7.1f} ({:10.3f} of "
                             "{:10d}) - {:6}    : {}",
                             perc, time, minTime, maxTime, avg, pd->count, key.c_str(), disName.c_str());
                }
            }
            ...
            LOG_INFO("playerbots",
                     "{:7.3f}% {:10.3f}s | {:7.1f} .. {:7.1f} ({:10.3f} of {:10d}) - "
                     "{:6}    : {}",
    ...
    else
    {
             ...
             i != data.end(); ++i)
        {
            ...
            for (auto& name : names)
            {
                ...
                {
                    LOG_INFO("playerbots",
                             "{:7.3f}% {:9.3f}ms | {:7.1f} .. {:7.1f} ({:10.3f} of "
                             "{:10.2f}) - {:6}    : {}",
                             perc, time, minTime, maxTime, avg, amount, key.c_str(), disName.c_str());
                }
            }
