...
std::string const RandomPlayerbotFactory::CreateRandomBotName(uint8 gender)
{
    ...
    while (--tries)
    {
        ...
        // Replace Catagory value with random Letter from that Catagory's Letter string for a given
        // bot gender
...
std::string const RandomPlayerbotFactory::CreateRandomGuildName()
{
    ...
    result = CharacterDatabase.Query(
        ...
        "LEFT OUTER JOIN guild e ON e.name = n.name WHERE e.guildid IS NULL AND n.name_id >= {} "
        "LIMIT 1",
        id);
    if (!result)
    {
...
std::string const RandomPlayerbotFactory::CreateRandomArenaTeamName()
{
    ...
    uint32 id = urand(0, maxId);
    result = CharacterDatabase.Query(
        "SELECT n.name FROM playerbots_arena_team_names n LEFT OUTER JOIN arena_team e ON e.name = "
        "n.name "
