                                                ...
                                                std::unordered_map<uint8, std::vector<std::string>> names)
{
    ...
    if (!player->Create(sObjectMgr->GetGenerator<HighGuid::Player>().Generate(), characterInfo.get()))
    {
        ...
        LOG_ERROR("playerbots",
                  "Unable to create random bot for account {} - name: \"{}\"; "
                  "race: {}; class: {}",
...
std::string const RandomPlayerbotFactory::CreateRandomBotName(uint8 gender)
{
    ...
    LOG_ERROR("playerbots",
              "No more names left for random bots. Attempting "
              "conlang name generation.");
    ...
    while (--tries)
    {
        ...
        // Replace Catagory value with random Letter from that Catagory's Letter
        // string for a given bot gender
...
void RandomPlayerbotFactory::CreateRandomBots()
{
    ...
    if (sPlayerbotAIConfig->deleteRandomBotAccounts)
    {
        ...
        /* TODO(yunfan): we need to sleep here to wait for async account deleted, or
           the newly account won't be created correctly the better way is turning
           the async db operation to sync db operation */
        std::this_thread::sleep_for(10ms * sPlayerbotAIConfig->randomBotAccountCount);
        LOG_INFO("playerbots", "Random bot characters deleted.");
        LOG_INFO("playerbots",
                 "Please reset the AiPlayerbot.DeleteRandomBotAccounts to 0 and "
                 "restart the server...");
    ...
    if (account_creation)
    {
        /* wait for async accounts create to make character create correctly, same
         * as account delete */
        std::this_thread::sleep_for(10ms * sPlayerbotAIConfig->randomBotAccountCount);
    }
...
void RandomPlayerbotFactory::CreateRandomGuilds()
{
    ...
    for (; guildNumber < sPlayerbotAIConfig->randomBotGuildCount; ++guildNumber)
    {
        ...
        {
            LOG_ERROR("playerbots",
                      "ObjectAccessor Cannot find player to set leader for guild {} "
                      ". Skipped...",
                      guildName.c_str());
            continue;
        }
...
std::string const RandomPlayerbotFactory::CreateRandomGuildName()
{
    ...
    result = CharacterDatabase.Query(
        ...
        "LEFT OUTER JOIN guild e ON e.name = n.name WHERE e.guildid IS NULL AND "
        "n.name_id >= {} LIMIT 1",
        id);
    if (!result)
    {
...
void RandomPlayerbotFactory::CreateRandomArenaTeams()
{
    ...
    for (; arenaTeamNumber < sPlayerbotAIConfig->randomBotArenaTeamCount; ++arenaTeamNumber)
    {
        ...
        QueryResult results = CharacterDatabase.Query(
            "SELECT `type` FROM playerbots_arena_team_names WHERE name = '{}'", arenaTeamName.c_str());
        if (!results)
        {
        ...
        // arenateam->SetStats(STAT_TYPE_WINS_WEEK, urand(0,
        // arenateam->GetStats().games_week));
        // arenateam->SetStats(STAT_TYPE_GAMES_SEASON,
        // urand(arenateam->GetStats().games_week, arenateam->GetStats().games_week
        // * 5)); arenateam->SetStats(STAT_TYPE_WINS_SEASON,
...
std::string const RandomPlayerbotFactory::CreateRandomArenaTeamName()
{
    ...
    uint32 id = urand(0, maxId);
    result = CharacterDatabase.Query(
        "SELECT n.name FROM playerbots_arena_team_names n LEFT OUTER JOIN "
        "arena_team e ON e.name = n.name "
