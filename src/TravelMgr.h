...
// A quest destination container for quick lookup of all destinations related to
// a quest.
...
public:
    ...
    // Quick square distance calculation without map check. Used for getting the
    // minimum distant points.
    ...
    // Returns the closest point of the list. Fast but only works for the same
    // map.
...
// TRAVEL    (move towards target until close enough) (rpg and grind is
// disabled) WORK      (grind/rpg until the target is no longer active) (rpg and
// grind is enabled on quest mobs) COOLDOWN  (wait some time free to do what the
// bot wants) EXPIRE    (if any of the above actions take too long pick a new
// target)
