...
// TravelNodePath(float distance = 0.1f, float extraCost = 0, TravelNodePathType pathType =
// TravelNodePathType::walk, uint32 pathObject = 0, bool calculated = false, std::vector<uint8>
// maxLevelCreature = { 0,0,0 }, float swimDistance = 0)
...
float TravelNodePath::getCost(Player *bot, uint32 cGold)
{
    ...
    if (bot)
    {
        ...
        bool isAlliance =
            Unit::GetFactionReactionTo(bot->GetFactionTemplateEntry(),
        ...
        {
            int mobAnnoyance =
                (maxLevelCreature[0] - level) - 10;  // Mobs 10 levels below do not bother us.

            if (isAlliance)
                factionAnnoyance = (maxLevelCreature[2] - level) -
                                   10;  // Opposite faction below 30 do not bother us.
            else if (!isAlliance)
            ...
            if (factionAnnoyance > 0)
                modifier +=
                    0.3 * factionAnnoyance;  // For each level the whole path takes 10% longer.
        }
    }
...
TravelNodePath *TravelNode::buildPath(TravelNode *endNode, Unit *bot, bool postProcess)
{
    ...
    if (!canPath &&
        endNode->hasLinkTo(this))  // Unable to find a path? See if the reverse is possible.
    {
        ...
        if (backNodePath.getPathType() == TravelNodePathType::walk)
        {
            ...
            if (!backNodePath.getComplete())  // Build it if it's not already complete.
            {
                ...
                bPath = thisPos->getPathFromPath(
                    bPath, bot);  // Pathfind from the existing path to the this Node.
    ...
    // Transports are (probably?) not solid at this moment. We need to walk over them so we need
    // extra code for this. Some portals are 'too' solid so we can't properly walk in them. Again we
    // need to bypass this.
    if (!isTransport() && !isPortal() && (endNode->isPortal() || endNode->isTransport()))
    {
        if (endNode->isTransport() && path.back().isInWater())  // Do not swim to boats.
            canPath = false;
                 ...
                 endPos->isPathTo(path, 20.0f))  // Cheat a little for transports and portals.
        {
    ...
    {
        WorldPosition secondPos = *std::next(
            path.begin());  // This is to prevent bots from jumping in the water from a transport.
                            // Need to remove this when transports are properly handled.
...
std::vector<TravelNode *> TravelNode::getNodeMap(bool importantOnly,
                                                 std::vector<TravelNode *> ignoreNodes)
{
    ...
    while (i < openList.size())
    {
        ...
        for (auto &nextPath : *currentNode->getLinks())
        {
            ...
            {
                if (ignoreNodes.empty() || std::find(ignoreNodes.begin(), ignoreNodes.end(),
                                                     nextNode) == ignoreNodes.end())
...
bool TravelNode::isUselessLink(TravelNode *farNode)
{
    ...
    for (auto &link : *getLinks())
    {
        ...
        else
        {
            ...
            // Is it quicker to go past second (and multiple) nodes to reach the first node instead
            // of going directly?
...
bool TravelNode::cropUselessLinks()
{
    ...
    for (auto& firstLink : getLinks())
    {
        ...
        for (auto& secondLink : getLinks())
        {
            ...
            if (std::find(toRemove.begin(), toRemove.end(), [firstNode,
    secondNode](std::pair<TravelNode*, TravelNode*> pair) {return pair.first == firstNode ||
    pair.first == secondNode;}) != toRemove.end()) continue;

            if (firstNode->hasLinkTo(secondNode))
            {
                //Is it quicker to go past first node to reach second node instead of going
    directly? if (firstLength + firstNode->linkLengthTo(secondNode) < secondLength * 1.1)
                {
            ...
            else
            {
                ...
                //Is it quicker to go past first (and multiple) nodes to reach the second node
    instead of going directly? if (firstLength + route.getLength() < secondLength * 1.1)
                {
                    if (secondNode->hasLinkTo(this) && !firstNode->hasLinkTo(this))
                        continue;
...
bool TravelPath::makeShortCut(WorldPosition startPos, float maxDist)
{
        ...
        if (p.point.getMapId() == startPos.getMapId())
        {
            ...
            if (curDist <
                sPlayerbotAIConfig->tooCloseDistance *
                    sPlayerbotAIConfig
                        ->tooCloseDistance)  // We are on the path. This is a good starting point
            {
                minDist = curDist;
                totalDist = curDist;
                newPath.clear();
            ...
            if (p.type !=
                NODE_PREPATH)  // Only look at the part after the first node and in the same map.
            {
                if (!firstNode)
                    firstNode = p.point;
                ...
                if (minDist == -1 || curDist < minDist ||
                    (curDist < maxDistSq &&
                     curDist < totalDist / 2))  // Start building from the last closest point or a
                                                // point that is close but far on the path.
                {
                    minDist = curDist;
                    totalDist = curDist;
                    newPath.clear();
    ...
    if (newPath.empty() || minDist > maxDistSq ||
        newPath.front().point.getMapId() != startPos.getMapId())
    {
        clear();
        return false;
    }
...
bool TravelPath::shouldMoveToNextPoint(WorldPosition startPos,
                                       ...
                                       std::vector<PathNodePoint>::iterator p, float &moveDist,
                                       float maxDist)
{
    ...
    if (p->point.getMapId() != startPos.getMapId() ||
        ((moveDist + nextMove > maxDist || startPos.distance(nextP->point) > maxDist) &&
         moveDist > 0))
    {
        return false;
    }
...
WorldPosition TravelPath::getNextPoint(WorldPosition startPos, float maxDist,
                                       TravelNodePathType &pathType, uint32 &entry)
{
    if (getPath().empty())
...
TravelPath TravelNodeRoute::buildPath(std::vector<WorldPosition> pathToStart,
                                      ...
                                      [[maybe_unused]] Unit *bot)
{
    ...
    for (auto &node : nodes)
    {
        if (prevNode)
        {
            ...
            if (!nodePath ||
                !nodePath->getComplete())  // Build the path to the next node if it doesn't exist.
            {
                if (!prevNode->isTransport())
                    nodePath = prevNode->buildPath(node, nullptr);
                else  // For transports we have no proper path since the node is in air/water.
                      // Instead we build a reverse path and follow that.
                {
            ...
            if (!nodePath ||
                !nodePath
                     ->getComplete())  // It looks like we can't properly path to our node. Make a
                                       // temporary reverse path and see if that works instead.
            {
                returnNodePath = *node->buildPath(
                    prevNode, nullptr);  // Build reverse path and save it to a temporary variable.
                std::vector<WorldPosition> path = returnNodePath.getPath();
            ...
            if (!nodePath ||
                !nodePath
                     ->getComplete())  // If we can not build a path just try to move to the node.
            {
            ...
            {
                travelPath.addPoint(*prevNode->getPosition(), NODE_PORTAL,
                                    nodePath->getPathObject());  // Entry point
                travelPath.addPoint(*node->getPosition(), NODE_PORTAL,
                                    nodePath->getPathObject());  // Exit point
            }
            else if (nodePath->getPathType() ==
                     TravelNodePathType::transport)  // Move onto transport
            {
                travelPath.addPoint(*prevNode->getPosition(), NODE_TRANSPORT,
                                    nodePath->getPathObject());  // Departure point
                travelPath.addPoint(*node->getPosition(), NODE_TRANSPORT,
                                    nodePath->getPathObject());  // Arrival point
            }
            else if (nodePath->getPathType() ==
                     TravelNodePathType::flightPath)  // Use the flightpath
            {
                travelPath.addPoint(*prevNode->getPosition(), NODE_FLIGHTPATH,
                                    nodePath->getPathObject());  // Departure point
                travelPath.addPoint(*node->getPosition(), NODE_FLIGHTPATH,
                                    nodePath->getPathObject());  // Arrival point
            }
            else if (nodePath->getPathType() == TravelNodePathType::teleportSpell)
            {
                travelPath.addPoint(*prevNode->getPosition(), NODE_TELEPORT,
            ...
            else
            {
                ...
                if (path.size() > 1 &&
                    node != nodes.back())  // Remove the last point since that will also be the
                                           // start of the next path.
                    path.pop_back();

                if (path.size() > 1 && prevNode->isPortal() &&
                    nodePath->getPathType() !=
                        TravelNodePathType::portal)  // Do not move to the area trigger if we don't
                                                     // plan to take the portal.
                    path.erase(path.begin());

                if (path.size() > 1 && prevNode->isTransport() &&
                    nodePath->getPathType() !=
                        TravelNodePathType::transport)  // Do not move to the transport if we aren't
                                                        // going to take it.
                    path.erase(path.begin());
...
std::ostringstream const TravelNodeRoute::print()
{
    ...
    for (auto &node : nodes)
    {
        out << std::fixed << node->getPosition()->getDisplayX() << " "
            << node->getPosition()->getDisplayY() << ",";
    }
...
TravelNode *TravelNodeMap::addNode(WorldPosition pos, std::string const preferedName,
                                   bool isImportant, bool checkDuplicate,
                                   [[maybe_unused]] bool transport,
...
TravelNode *TravelNodeMap::getNode(WorldPosition pos,
                                   [[maybe_unused]] std::vector<WorldPosition> &ppath, Unit *bot,
                                   float range)
{
    float x = pos.getX();
    float y = pos.getY();
...
TravelNodeRoute TravelNodeMap::getRoute(TravelNode *start, TravelNode *goal, Player *bot)
{
    ...
    TravelNodeStub *startStub =
        &m_stubs.insert(std::make_pair(start, TravelNodeStub(start))).first->second;
    ...
    if (bot)
    {
        ...
        if (botAI)
        {
            ...
            else
            {
                ...
                startStub->currentGold =
                    AI_VALUE2(uint32, "free money for", (uint32)NeedMoneyFor::travel);
            }
        }
        else
        ...
        if (!bot->HasSpellCooldown(8690) && bot->IsAlive())
        {
            ...
            TravelNode *homeNode =
                sTravelNodeMap->getNode(AI_VALUE(WorldPosition, "home bind"), nullptr, 10.0f);
            if (homeNode)
            {
                PortalNode *portNode =
                    (PortalNode *)sTravelNodeMap->teleportNodes[bot->GetGUID()][8690];
                {
                ...
                childNode = &m_stubs.insert(std::make_pair(portNode, TravelNodeStub(portNode)))
                                 .first->second;
    ...
    while (!open.empty())
    {
        ...
        if (currentNode->dataNode == goal ||
            (currentNode->dataNode->getMapId() != start->getMapId() &&
        ...
        for (const auto &link : *currentNode->dataNode->getLinks())  // for each successor n' of n
        {
            ...
            childNode =
                &m_stubs.insert(std::make_pair(linkNode, TravelNodeStub(linkNode))).first->second;
                                        ...
                                        std::vector<WorldPosition> &startPath, Player *bot)
{
    ...
    while (startI < 5 && endI < 5)
    {
        ...
        float maxStartDistance =
            startNode->isTransport() ? 20.0f : sPlayerbotAIConfig->targetPosRecalcDistance;
        ...
        if (!route.isEmpty())
        {
            ...
            if (startNodePosition.cropPathTo(newStartPath, maxStartDistance) ||
                startNode->getPosition()->isPathTo(
                    newStartPath = startPos.getPathTo(startNodePosition, nullptr),
                    maxStartDistance))
            {
...
TravelPath TravelNodeMap::getFullPath(WorldPosition startPos, WorldPosition endPos, Player *bot)
{
    ...
    // Find the route of nodes starting at a node closest to the start position and ending at a node
    // closest to the endposition. Also returns longPath: The path from the start position to the
...
void TravelNodeMap::generateNpcNodes()
{
    ...
    {
        WorldPosition guidP(creatureData->mapid, creatureData->posX, creatureData->posY,
                            creatureData->posZ, creatureData->orientation);

        CreatureTemplate const *cInfo = sObjectMgr->GetCreatureTemplate(creatureData->id1);
        if (!cInfo)
            continue;

        uint32 flagMask = UNIT_NPC_FLAG_INNKEEPER | UNIT_NPC_FLAG_FLIGHTMASTER |
...
void TravelNodeMap::generateStartNodes()
{
    ...
    for (uint32 i = 0; i < MAX_RACES; i++)
    {
        for (uint32 j = 0; j < MAX_CLASSES; j++)
        {
            ...
            WorldPosition pos(info->mapId, info->positionX, info->positionY, info->positionZ,
                              info->orientation);
...
void TravelNodeMap::generateAreaTriggerNodes()
{
    ...
    for (auto const &itr : sObjectMgr->GetAllAreaTriggerTeleports())
    {
        ...
        WorldPosition outPos =
            WorldPosition(atEntry.target_mapId, atEntry.target_X, atEntry.target_Y,
                          atEntry.target_Z, atEntry.target_Orientation);
    ...
    for (auto const &itr : sObjectMgr->GetAllAreaTriggerTeleports())
    {
        ...
        TravelNode *entryNode =
            sTravelNodeMap->getNode(outPos, nullptr, 20.0f);  // Entry side, portal exit.

        TravelNode *outNode =
            sTravelNodeMap->addNode(outPos, nodeName, true, true);  // Exit size, portal exit.

        TravelNode *inNode =
            sTravelNodeMap->getNode(inPos, nullptr, 5.0f);  // Entry side, portal center.

        // Portal link from area trigger to area trigger destination.
        if (outNode && inNode)
        {
            TravelNodePath travelPath(0.1f, 3.0f, (uint8)TravelNodePathType::portal, itr.first,
                                      true);
...
void TravelNodeMap::generateTransportNodes()
{
    for (auto const &itr : *sObjectMgr->GetGameObjectTemplates())
    {
        ...
        if (data &&
            (data->type == GAMEOBJECT_TYPE_TRANSPORT || data->type == GAMEOBJECT_TYPE_MO_TRANSPORT))
        {
            ...
            if (path.empty())
            {
                if (animation)
                {
                    ...
                    {
                        prevNode = nullptr;
                        WorldPosition basePos(transport->mapid, transport->posX, transport->posY,
                                              transport->posZ, transport->orientation);
                        ...
                        for (auto &p : aPath)
                        {
                            ...
                            WorldPosition pos = WorldPosition(
                                basePos.getMapId(), basePos.getX() + dx, basePos.getY() + dy,
                            ...
                            {
                                TravelNode *node = sTravelNodeMap->addNode(pos, data->name, true,
                                                                           true, true, itr.first);
                                ...
                                else
                                {
                                    ...
                                    TravelNodePath travelPath(0.1f, totalTime,
                                                              (uint8)TravelNodePathType::transport,
                                                              itr.first, true);
                        ...
                        if (prevNode)
                        {
                            for (auto &p : aPath)
                            {
                                ...
                                if (pos.distance(&lPos) == 0)
                                {
                                    TravelNode *node = sTravelNodeMap->addNode(
                                        pos, data->name, true, true, true, itr.first);
                                    if (node != prevNode)
                                    {
                                        ...
                                        TravelNodePath travelPath(
                                            0.1f, totalTime, (uint8)TravelNodePathType::transport,
                                            itr.first, true);
            ...
            else  // Boats/Zepelins
            {
                ...
                for (auto &p : path)
                {
                    ...
                    if (p->delay > 0)
                    {
                        ...
                        {
                            TravelNodePath travelPath(
                                0.1f, 0.0, (uint8)TravelNodePathType::transport, itr.first, true);
                ...
                if (prevNode)
                {
                    ...
                    for (auto &p : path)
                    {
                        ...
                        if (p->delay > 0)
                        {
                            ...
                            if (node != prevNode)
                            {
                                TravelNodePath travelPath(0.1f, 0.0,
                                                          (uint8)TravelNodePathType::transport,
                                                          itr.first, true);
                                travelPath.setPathAndCost(ppath, moveSpeed);
...
{
    if (!sPlayerbotAIConfig->hasLog("travelNodes.csv") &&
        !sPlayerbotAIConfig->hasLog("travelPaths.csv"))
        return;
...
void TravelNodeMap::printNodeStore()
{
    ...
    sPlayerbotAIConfig->log(nodeStore, "    {");
    sPlayerbotAIConfig->log(nodeStore, "        TravelNode** nodes = new TravelNode*[%zu];",
                            anodes.size());
    ...
    for (uint32 i = 0; i < anodes.size(); i++)
    {
        ...
        //        struct addNode {uint32 node; WorldPosition point; std::string const name; bool
        //        isPortal; bool isTransport; uint32 transportId; };
        out << std::fixed << std::setprecision(2) << "        addNodes.push_back(addNode{" << i
            << ",";
        out << "WorldPosition(" << node->getMapId() << ", " << node->getX() << "f, " << node->getY()
        ...
        /*
           ...
           sTravelNodeMap->addNode(&WorldPosition(" << node->getMapId() << "," << node->getX() <<
           "f," << node->getY() << "f," << node->getZ() << "f,"<< node->getO() <<"f), \""
                    << name << "\", " << (node->isImportant() ? "true" : "false") << ", true";
                if (node->isTransport())
                    out << "," << (node->isTransport() ? "true" : "false") << "," <<
           node->getTransportId();
    ...
    for (uint32 i = 0; i < anodes.size(); i++)
    {
        ...
        for (auto &Link : *node->getLinks())
        {
            ...
            //        struct linkNode { uint32 node1; uint32 node2; float distance; float extraCost;
            //        bool isPortal; bool isTransport; uint32 maxLevelMob; uint32 maxLevelAlliance;
            //        uint32 maxLevelHorde; float swimDistance; };

            out << std::fixed << std::setprecision(2) << "        linkNodes3.push_back(linkNode3{"
                << i << "," << saveNodes.find(Link.first)->second << ",";
            out << Link.second->print() << "});";

            // out << std::fixed << std::setprecision(1) << "        nodes[" << i <<
            // "]->setPathTo(nodes[" << saveNodes.find(Link.first)->second << "],TravelNodePath(";
...
void TravelNodeMap::saveNodeStore()
{
    ...
    for (uint32 i = 0; i < anodes.size(); i++)
    {
        ...
        PlayerbotsDatabasePreparedStatement *stmt =
            PlayerbotsDatabase.GetPreparedStatement(PLAYERBOTS_INS_TRAVELNODE);
        stmt->SetData(0, i);
        stmt->SetData(1, name);
...
void TravelNodeMap::loadNodeStore()
{
...

    {
        if (PreparedQueryResult result = PlayerbotsDatabase.Query(
                PlayerbotsDatabase.GetPreparedStatement(PLAYERBOTS_SEL_TRAVELNODE)))
        {
            do
            {
                ...
                TravelNode *node =
                    addNode(WorldPosition(fields[2].Get<uint32>(), fields[3].Get<float>(),
    ...
    {
        if (PreparedQueryResult result = PlayerbotsDatabase.Query(
                PlayerbotsDatabase.GetPreparedStatement(PLAYERBOTS_SEL_TRAVELNODE_LINK)))
        {
            do
            {
                Field *fields = result->Fetch();
                ...
                startNode->setPathTo(
                    endNode,
                    TravelNodePath(
                        fields[4].Get<float>(), fields[6].Get<float>(), fields[2].Get<uint8>(),
    ...
    {
        if (PreparedQueryResult result = PlayerbotsDatabase.Query(
                PlayerbotsDatabase.GetPreparedStatement(PLAYERBOTS_SEL_TRAVELNODE_PATH)))
        {
            do
            {
                Field *fields = result->Fetch();
                ...
                std::vector<WorldPosition> ppath = path->getPath();
                ppath.push_back(WorldPosition(fields[3].Get<uint32>(), fields[4].Get<float>(),
...
void TravelNodeMap::calcMapOffset()
{
    ...
    for (auto &mapId : mapIds)
    {
        mapOffsets.push_back(
            std::make_pair(mapId, WorldPosition(mapId, curPos.getX() - min[i].getX(),
                                                curPos.getY() - max[i].getY(), 0, 0)));
