...
// TravelNodePath(float distance = 0.1f, float extraCost = 0, TravelNodePathType
// pathType = TravelNodePathType::walk, uint32 pathObject = 0, bool calculated =
// false, std::vector<uint8> maxLevelCreature = { 0,0,0 }, float swimDistance =
// 0)
...
TravelNodePath* TravelNode::buildPath(TravelNode* endNode, Unit* bot, bool postProcess)
{
    ...
    if (!canPath && endNode->hasLinkTo(this))  // Unable to find a path? See if the reverse is possible.
    {
        ...
        if (backNodePath.getPathType() == TravelNodePathType::walk)
        {
            ...
            {
                if (bPath.empty())
                    bPath = {*endNode->getPosition()};  // Start the path from the end
                                                        // Node.
    ...
    // Transports are (probably?) not solid at this moment. We need to walk over
    // them so we need extra code for this. Some portals are 'too' solid so we
    // can't properly walk in them. Again we need to bypass this.
    ...
    {
        WorldPosition secondPos = *std::next(path.begin());  // This is to prevent bots from jumping in the
                                                             // water from a transport. Need to remove this
                                                             // when transports are properly handled.
...
bool TravelNode::isUselessLink(TravelNode* farNode)
{
    ...
    for (auto& link : *getLinks())
    {
        ...
        {
            // Is it quicker to go past second node to reach first node instead of
            // going directly?
        ...
        else
        {
            ...
            // Is it quicker to go past second (and multiple) nodes to reach the first
            // node instead of going directly?
...
bool TravelNode::cropUselessLinks()
{
    ...
    secondNode](std::pair<TravelNode*, TravelNode*> pair) {return pair.first ==
    firstNode || pair.first == secondNode;}) != toRemove.end()) continue;

            if (firstNode->hasLinkTo(secondNode))
            {
                //Is it quicker to go past first node to reach second node instead
    of going directly? if (firstLength + firstNode->linkLengthTo(secondNode) <
    secondLength * 1.1)
                {
                    if (secondNode->hasLinkTo(this) &&
    !firstNode->hasLinkTo(this)) continue;
            ...
            {
                TravelNodeRoute route = sTravelNodeMap->getRoute(firstNode,
    secondNode, false);
                ...
                //Is it quicker to go past first (and multiple) nodes to reach the
    second node instead of going directly? if (firstLength + route.getLength() <
    secondLength * 1.1)
                {
                    if (secondNode->hasLinkTo(this) &&
    !firstNode->hasLinkTo(this)) continue;
    ...
    firstNode || pair.first == secondNode; }) != toRemove.end()) continue;
                ...
                if (firstNode->hasLinkTo(secondNode))
                {
                    //Is it quicker to go past first node to reach second node
    instead of going directly? if (firstLength +
    firstNode->linkLengthTo(secondNode) < secondLength * 1.1)
                    {
                        if (secondNode->hasLinkTo(this) &&
    !firstNode->hasLinkTo(this)) continue;
    ...
    secondNode, false);
                    ...
                    //Is it quicker to go past first (and multiple) nodes to reach
    the second node instead of going directly? if (firstLength + route.getLength()
    < secondLength * 1.1)
                    {
                        if (secondNode->hasLinkTo(this) &&
    !firstNode->hasLinkTo(this)) continue;
...
bool TravelPath::makeShortCut(WorldPosition startPos, float maxDist)
{
    ...
    for (auto& p : fullPath)  // cycle over the full path
    {
        ...
        if (p.point.getMapId() == startPos.getMapId())
        {
            ...
            if (curDist < sPlayerbotAIConfig->tooCloseDistance *
                              sPlayerbotAIConfig->tooCloseDistance)  // We are on the path. This
            ...
            if (p.type != NODE_PREPATH)  // Only look at the part after the first node
                                         // and in the same map.
            {
                if (!firstNode)
                    firstNode = p.point;
                                       ...
                                       float& moveDist, float maxDist)
{
    ...
    // We are moving to a area trigger node and want to move to the next teleport
    // node.
                                       ...
                                       uint32& entry)
{
    ...
    // We are moving towards a flight path. Move to flight master and activate
    // flight path.
    ...
    if (moveDist < sPlayerbotAIConfig->targetPosRecalcDistance && std::next(startP) != ed)
    {
        ...
        // startPos.getPathTo(std::next(startP)->point, nullptr); startP->point =
        // startPos.lastInRange(path, -1, maxDist);
        return WorldPosition();
    }
                                      ...
                                      [[maybe_unused]] Unit* bot)
{
    ...
    for (auto& node : nodes)
    {
        if (prevNode)
        {
            ...
            if (!nodePath || !nodePath->getComplete())  // Build the path to the next
                                                        // node if it doesn't exist.
            {
                if (!prevNode->isTransport())
                    nodePath = prevNode->buildPath(node, nullptr);
                else  // For transports we have no proper path since the node is in
                      // air/water. Instead we build a reverse path and follow that.
                {
                    node->buildPath(prevNode,
                                    nullptr);  // Reverse build to get proper path.
                    nodePath = prevNode->getPathTo(node);
                }
            }
            ...
            if (!nodePath || !nodePath->getComplete())  // It looks like we can't properly path to
                                                        // our node. Make a temporary reverse path
                                                        // and see if that works instead.
            {
                returnNodePath = *node->buildPath(prevNode,
                                                  nullptr);  // Build reverse path and save it to a temporary variable.
                std::vector<WorldPosition> path = returnNodePath.getPath();
            ...
            if (!nodePath || !nodePath->getComplete())  // If we can not build a path just try to
                                                        // move to the node.
            {
            ...
            else
            {
                ...
                if (path.size() > 1 && node != nodes.back())  // Remove the last point since that will also
                                                              // be the start of the next path.
                    path.pop_back();
                ...
                if (path.size() > 1 && prevNode->isTransport() &&
                    nodePath->getPathType() != TravelNodePathType::transport)  // Do not move to the transport
                                                                               // if we aren't going to take it.
                    path.erase(path.begin());
...
TravelPath TravelNodeMap::getFullPath(WorldPosition startPos, WorldPosition endPos, Player* bot)
{
    ...
    // We try to find nodes near the bot and near the end position that have a
    // route between them. Then bot has to move towards/along the route.
    sTravelNodeMap->m_nMapMtx.lock_shared();

    // Find the route of nodes starting at a node closest to the start position
    // and ending at a node closest to the endposition. Also returns longPath: The
    // path from the start position to the first node in the route.
...
void TravelNodeMap::printNodeStore()
{
    ...
    for (uint32 i = 0; i < anodes.size(); i++)
    {
        ...
        //        struct addNode {uint32 node; WorldPosition point; std::string
        //        const name; bool isPortal; bool isTransport; uint32 transportId;
        //        };
        ...
        /*
           ...
           << "] = sTravelNodeMap->addNode(&WorldPosition(" << node->getMapId() <<
           "," << node->getX() << "f," << node->getY() << "f," << node->getZ() <<
           "f,"<< node->getO() <<"f), \""
                    << name << "\", " << (node->isImportant() ? "true" : "false") <<
           ", true"; if (node->isTransport()) out << "," << (node->isTransport() ?
           "true" : "false") << "," << node->getTransportId();
    ...
    for (uint32 i = 0; i < anodes.size(); i++)
    {
        ...
        for (auto& Link : *node->getLinks())
        {
            ...
            //        struct linkNode { uint32 node1; uint32 node2; float distance;
            //        float extraCost; bool isPortal; bool isTransport; uint32
            //        maxLevelMob; uint32 maxLevelAlliance; uint32 maxLevelHorde;
            //        float swimDistance; };
            ...
            // "]->setPathTo(nodes[" << saveNodes.find(Link.first)->second <<
            // "],TravelNodePath("; out << Link.second->print() << "), true);";
            sPlayerbotAIConfig->log(nodeStore, out.str().c_str());
        }
    }
