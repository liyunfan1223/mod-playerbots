...
void PlayerbotFactory::Init()
{
    ...
    for (uint32 id = 1; id < maxStoreSize; ++id)
    {
        ...
        for (uint8 j = 0; j < MAX_SPELL_EFFECTS; ++j)
        {
            ...
            // SpellInfo const* enchantSpell =
            // sSpellMgr->GetSpellInfo(enchant->spellid[0]);
            // if (!enchantSpell)
            //     continue;
...
void PlayerbotFactory::Randomize(bool incremental)
{
    ...
    // LOG_DEBUG("playerbots", "Preparing to {} randomize...", (incremental ?
    // "incremental" : "full"));
    Prepare();
    ...
    "PlayerbotFactory_Immersive"); LOG_INFO("playerbots", "Initializing
    immersive..."); InitImmersive(); if (pmo) pmo->finish();
    */
    ...
    // {
    //     pmo = sPerformanceMonitor->start(PERF_MON_RNDBOT,
    //     "PlayerbotFactory_Enchant"); LOG_INFO("playerbots", "Initializing
    //     enchant templates..."); LoadEnchantContainer(); if (pmo)
    //         pmo->finish();
    // }
    ...
    // {
    // pmo = sPerformanceMonitor->start(PERF_MON_RNDBOT,
    // "PlayerbotFactory_EnchantTemplate"); LOG_INFO("playerbots", "Initializing
    // enchant templates..."); ApplyEnchantTemplate(); if (pmo)
    //     pmo->finish();
    // }
    ...
    // pmo = sPerformanceMonitor->start(PERF_MON_RNDBOT,
    // "PlayerbotFactory_Guilds"); LOG_INFO("playerbots", "Initializing
    // guilds..."); bot->SaveToDB(false, false); InitGuild(); bot->SaveToDB(false,
    // false);
    // if (pmo)
    //    pmo->finish();
    ...
    // {
    //     pmo = sPerformanceMonitor->start(PERF_MON_RNDBOT,
    //     "PlayerbotFactory_Arenas"); LOG_INFO("playerbots", "Initializing arena
    //     teams..."); InitArenaTeam(); if (pmo)
    //         pmo->finish();
    // }
...
void PlayerbotFactory::InitPetTalents()
{
    ...
    if (!pet)
    {
        // LOG_INFO("playerbots", "{} init pet talents failed with no pet",
        // bot->GetName().c_str());
        return;
    }
    ...
    if (!ci)
    {
        // LOG_INFO("playerbots", "{} init pet talents failed with no creature
        // template", bot->GetName().c_str());
        return;
    }
    CreatureFamilyEntry const* pet_family = sCreatureFamilyStore.LookupEntry(ci->family);
    if (!pet_family || pet_family->petTalentType < 0)
    {
        // LOG_INFO("playerbots", "{} init pet talents failed with petTalentType <
        // 0({})", bot->GetName().c_str(), pet_family->petTalentType);
        return;
    }
    // pet->resetTalents();
    ...
    int row = 0;
    // LOG_INFO("playerbots", "{} learning, max talent points: {}, cur: {}",
    // bot->GetName().c_str(), maxTalentPoints, curTalentPoints);
    for (auto i = spells.begin(); i != spells.end(); ++i, ++row)
    {
               ...
               attemptCount++ < 10 && pet->GetFreeTalentPoints())
        {
            ...
            // LOG_INFO("playerbots", "{} learn pet talent {}({})",
            // bot->GetName().c_str(), talentInfo->TalentID, maxRank);
...
void PlayerbotFactory::InitPet()
{
    ...
    // LOG_INFO("playerbots", "Start make spell auto cast for {} spells. {}
    // already auto casted.", pet->m_spells.size(), pet->GetPetAutoSpellSize());
...
bool PlayerbotFactory::CanEquipArmor(ItemTemplate const* proto)
{
    ...
    //     if (slot == EQUIPMENT_SLOT_OFFHAND && bot->getClass() == CLASS_PALADIN
...
bool PlayerbotFactory::CanEquipItem(ItemTemplate const* proto, uint32 desiredQuality)
{
    ...
    // else if (proto->Class == ITEM_CLASS_WEAPON || proto->SubClass ==
...
//         uint32 attempts = 10;
//         if (urand(0, 100) < 100 *
//         sPlayerbotAIConfig->randomGearLoweringChance && quality >
//         ITEM_QUALITY_NORMAL) {
//             quality--;
//         }
//         // current item;
...
//             if (isUpgrade)
//             {
//                 std::vector<uint32> ids = sRandomItemMgr->GetUpgradeList(bot,
//                 specName, slot, 0, itemInSlot); if (!ids.empty())
//                     Shuffle(ids);
...
//                         EnchantItem(newItem);
//                         LOG_INFO("playerbots", "Bot {} {}:{} <{}>: Equip: {},
//                         slot: {}, Old item: {}",
//                             bot->GetGUID().ToString().c_str(),
//                             IsAlliance(bot->getRace()) ? "A" : "H",
...
//             {
//                 std::vector<uint32> ids = sRandomItemMgr->GetUpgradeList(bot,
//                 specName, slot, quality, itemInSlot); if (!ids.empty())
//                     Shuffle(ids);
...
//                             bot->GetGUID().ToString().c_str(),
//                             IsAlliance(bot->getRace()) ? "A" : "H",
//                             bot->GetLevel(), bot->GetName(), newItemId,
//                             slot);
//                         break;
//                     }
//                 }
...
//                 bot->GetGUID().ToString().c_str(), IsAlliance(bot->getRace())
//                 ? "A" : "H", bot->GetLevel(), bot->GetName(), slot);
//             continue;
//         }
//     }
// }
...
//     uint32 desiredQuality = itemQuality;
//     while (urand(0, 100) < 100 * sPlayerbotAIConfig->randomGearLoweringChance
//     && desiredQuality > ITEM_QUALITY_NORMAL)
//     {
//         desiredQuality--;
//     }

//     ItemTemplateContainer const* itemTemplate =
//     sObjectMgr->GetItemTemplateStore(); do
//     {
...
//                 {
//                     if (!sRandomItemMgr->GetLiveStatWeight(bot,
//                     proto->ItemId))
//                         continue;
//                 }
...
//                             if (proto->SubClass == ITEM_SUBCLASS_WEAPON_AXE
...
//                     if (existingItem->GetTemplate()->SubClass ==
//                     ITEM_SUBCLASS_ARMOR_SHIELD)
//                         continue;
//             }
//             else
...
//         }
//     } while (items[ITEM_CLASS_ARMOR].empty() &&
//     items[ITEM_CLASS_WEAPON].empty() && desiredQuality-- >
//     ITEM_QUALITY_NORMAL);

//     for (std::map<uint32, std::vector<uint32>>::iterator i = items.begin(); i
//     != items.end(); ++i)
//     {
//         std::vector<uint32>& ids = i->second;
//         if (ids.empty())
//         {
//             LOG_DEBUG("playerbots",   "{}: no items to make second equipment
//             set for slot {}", bot->GetName().c_str(), i->first); continue;
//         }
...
//             if (Item* newItem = StoreNewItemInInventorySlot(bot, newItemId,
//             1))
//             {
//                 newItem->SetOwnerGUID(bot->GetGUID());
//                 EnchantItem(newItem);
...
{
    // if
    // (sPlayerbotAIConfig->parsedSpecLinkOrder[bot->getClass()][specNo][80].size()
    // == 0) {
    //     return;
    // }
    uint32 cls = bot->getClass();
...
void PlayerbotFactory::InitPotions()
{
    ...
    for (uint8 i = 0; i < 2; ++i)
    {
        ...
        if (!itemId)
        {
            // LOG_INFO("playerbots", "No potions (type {}) available for bot {} ({}
            // level)", effect, bot->GetName().c_str(), bot->GetLevel());
            continue;
        }
...
void PlayerbotFactory::ApplyEnchantTemplate(uint8 spec)
{
    for (EnchantContainer::const_iterator itr = GetEnchantContainerBegin(); itr != GetEnchantContainerEnd(); ++itr)
        if ((*itr).ClassId == bot->getClass() && (*itr).SpecId == spec)
        {
            ...
            if (!enchantid)
            {
                // LOG_ERROR("playerbots", "{}: Invalid enchantid ", enchantid, " report
                // to devs", bot->GetName().c_str());
                return;
            }
    ...
    // const SpellItemEnchantmentEntry* a =
    // sSpellItemEnchantmentStore.LookupEntry(1);
}
...
float PlayerbotFactory::CalculateItemScore(uint32 item_id, Player* bot)
{
    ...
    for (uint8 j = 0; j < MAX_ITEM_PROTO_SPELLS; j++)
    {
        ...
        // sSpellMgr->GetSpellInfo(proto->Spells[j].SpellId); if (!spellInfo)
        //     continue;
        // // spell category check?
        // for (uint8 i = 0; i < 3; i++)
        // {
        //     float multiplier = proto->Spells[j].SpellTrigger ==
        //     ITEM_SPELLTRIGGER_ON_EQUIP ? 1.0f : 0.2f; if
        ...
        //             {
        //                 for (uint32 rating = CR_WEAPON_SKILL; rating <
        //                 MAX_COMBAT_RATING; ++rating)
        //                 {
        ...
        //                                 defense +=
        //                                 (spellInfo->Effects[i].BasePoints + 1) *
        //                                 multiplier; break;
        //                             case CR_DODGE:
        //                                 dodge +=
        //                                 (spellInfo->Effects[i].BasePoints + 1) *
        ...
        //                         {
        //                             switch
        //                             (triggerSpellInfo->Effects[k].ApplyAuraName)
        //                             {
        //                                 case SPELL_AURA_MOD_DAMAGE_DONE:
        //                                 // case SPELL_AURA_MOD_HEALING_DONE:
        //                                 duplicated
        //                                     spell_power +=
        //                                     (spellInfo->Effects[k].BasePoints +
        //                                     1) * multiplier; break;
        //                                 case SPELL_AURA_MOD_ATTACK_POWER:
        //                                     attack_power +=
        //                                     (spellInfo->Effects[k].BasePoints +
    ...
    if (proto->Class == ITEM_CLASS_WEAPON)
    {
        ...
        // enhancement, rogue, ice dk, unholy dk, shield tank, fury warrior without
        // titan's grip but with duel wield
        if (isDoubleHand &&
    ...
    for (int s = 0; s < MAX_SPELL_ITEM_ENCHANTMENT_EFFECTS; ++s)
    {
        ...
        switch (enchant_display_type)
        {
            ...
            case ITEM_ENCHANTMENT_TYPE_EQUIP_SPELL:
            {
                ...
                //         (basepoints depends from item suffix factor) if
            ...
            case ITEM_ENCHANTMENT_TYPE_RESISTANCE:
                ...
                // enchant_spell_id), TOTAL_VALUE, float(enchant_amount), apply);
                break;
            ...
            case ITEM_ENCHANTMENT_TYPE_TOTEM:  // Shaman Rockbiter Weapon
            {
                ...
                //     if (item->GetSlot() == EQUIPMENT_SLOT_MAINHAND)
                //     {
                //         addValue = float(enchant_amount * item->GetTemplate()->Delay /
                //         1000.0f); HandleStatModifier(UNIT_MOD_DAMAGE_MAINHAND,
                //         TOTAL_VALUE, addValue, apply);
                //     }
                //     else if (item->GetSlot() == EQUIPMENT_SLOT_OFFHAND)
                //     {
                //         addValue = float(enchant_amount * item->GetTemplate()->Delay /
                //         1000.0f); HandleStatModifier(UNIT_MOD_DAMAGE_OFFHAND,
                //         TOTAL_VALUE, addValue, apply);
                //     }
                // }
                break;
            ...
            case ITEM_ENCHANTMENT_TYPE_PRISMATIC_SOCKET:
                ...
                break;
            default:
                // LOG_ERROR("entities.player", "Unknown item enchantment (id = {})
                // display type: {}", enchant_id, enchant_display_type);
                break;
        } /*switch (enchant_display_type)*/
    }
    int tab = AiFactory::GetPlayerSpecTab(bot);
