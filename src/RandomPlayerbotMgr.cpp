...
void RandomPlayerbotMgr::LogPlayerLocation()
{
    ...
    {
        return;
        // This is to prevent some thread-unsafeness. Crashes would happen if bots
        // get added or removed. We really don't care here. Just skip a log. Making
        // this thread-safe is not worth the effort.
    }
}
...
void RandomPlayerbotMgr::UpdateAIInternal(uint32 elapsed, bool /*minimal*/)
{
    ...
    if (sPlayerbotAIConfig->enablePrototypePerformanceDiff)
    {
        LOG_INFO("playerbots", "---------------------------------------");
        LOG_INFO("playerbots",
                 "PROTOTYPE: Playerbot performance enhancements are "
                 "active. Issues and instability may occur.");
        LOG_INFO("playerbots", "---------------------------------------");
        ScaleBotActivity();
    }
...
void RandomPlayerbotMgr::ScaleBotActivity()
{
    ...
    // if (activityPercentage >= 100.0f || activityPercentage <= 0.0f)
    // pid.reset(); //Stop integer buildup during max/min activity
...
void RandomPlayerbotMgr::CheckBgQueue()
{
            ...
            if (uint8 arenaType = BattlegroundMgr::BGArenaType(queueTypeId))
            {
                ...
                if (player->InArena())
                {
                    if (player->GetBattleground()->isRated() &&
                        (ginfo.IsRated && ginfo.ArenaTeamId && ginfo.ArenaTeamRating && ginfo.OpponentsTeamRating))
                        teamId = TEAM_HORDE;
                    else
            ...
            if (!player->IsInvitedForBattlegroundInstance() &&
                (!player->InBattleground() ||
                 (player->GetBattleground() &&
                  player->GetBattleground()->GetBgTypeID() != BattlegroundMgr::BGTemplateId(queueTypeId))))
            {
    ...
    for (uint8 i = BG_BRACKET_ID_FIRST; i < MAX_BATTLEGROUND_BRACKETS; ++i)
    {
        for (uint8 j = BATTLEGROUND_QUEUE_AV; j < MAX_BATTLEGROUND_QUEUE_TYPES; ++j)
        {
            ...
            if ((BgPlayers[j][i][TEAM_ALLIANCE] + BgBots[j][i][TEAM_ALLIANCE] + BgPlayers[j][i][TEAM_HORDE] +
                 BgBots[j][i][TEAM_HORDE]) == 0)
                continue;
            ...
            {
                LOG_INFO("playerbots",
                         "ARENA:{} {}: Player (Skirmish:{}, Rated:{}) B (Skirmish:{}, "
                         "Rated:{}) Total (Skirmish:{} Rated:{})",
                         ...
                         : i == 6 ? (i == 6 && MAX_BATTLEGROUND_BRACKETS == 16) ? "70-79" : "70"
                                  : "80",
                         BgPlayers[j][i][TEAM_ALLIANCE], BgPlayers[j][i][TEAM_HORDE], BgBots[j][i][TEAM_ALLIANCE],
                         BgBots[j][i][TEAM_HORDE], BgPlayers[j][i][TEAM_ALLIANCE] + BgBots[j][i][TEAM_ALLIANCE],
            ...
            LOG_INFO("playerbots", "BG:{} {}: Player ({}:{}) Bot ({}:{}) Total (A:{} H:{})", _bgType,
                     i == 0                                       ? "10-19"
                     : i == 1                                     ? "20-29"
                     : i == 2                                     ? "30-39"
                     : i == 3                                     ? "40-49"
...
bool RandomPlayerbotMgr::ProcessBot(uint32 bot)
{
    ...
    if (!player)
    {
        ...
        // do not randomize or teleport immediately after server start (prevent
        // lagging)
    ...
    if (!update)
    {
        ...
        if (botAI)
        {
            // botAI->GetAiObjectContext()->GetValue<bool>("random bot
            // update")->Set(true);
...
bool RandomPlayerbotMgr::ProcessBot(Player* player)
{
    ...
    //         {
    //             for (std::vector<Player*>::iterator i = players.begin(); i !=
    //             players.end(); ++i)
    //                 sGuildTaskMgr->Update(*i, player);
    //         }

    //         uint32 accountId =
    //         sCharacterCache->GetCharacterAccountIdByGuid(guild->GetLeaderGUID());
...
void RandomPlayerbotMgr::RandomTeleport(Player* bot, std::vector<WorldLocation>& locs, bool hearth)
{
    ...
    for (uint32 i = 0; i < tlocs.size(); i++)
    {
        ...
        float x = loc.GetPositionX();  // + (attemtps > 0 ? urand(0,
                                       // sPlayerbotAIConfig->grindDistance) -
                                       // sPlayerbotAIConfig->grindDistance / 2 : 0);
        float y = loc.GetPositionY();  // + (attemtps > 0 ? urand(0,
                                       // sPlayerbotAIConfig->grindDistance) -
                                       // sPlayerbotAIConfig->grindDistance / 2 : 0);
        ...
        LOG_INFO("playerbots",
                 "Random teleporting bot {} (level {}) to Map: {} ({}) Zone: {} "
                 "({}) Area: {} ({}) {},{},{} ({}/{} locations)",
...
void RandomPlayerbotMgr::PrepareTeleportCache()
{
    ...
    // sFactionTemplateStore.GetNumRows()); results = WorldDatabase.Query("SELECT
    // map, position_x, position_y, position_z, r.race, r.minl, r.maxl FROM
    ...
    //         sPlayerbotAIConfig->randomBotMaxLevel + 1; level++)
    //         {
...
uint32 RandomPlayerbotMgr::GetZoneLevel(uint16 mapId, float teleX, float teleY, float teleZ)
{
    ...
    uint32 level = 0;
    QueryResult results = WorldDatabase.Query(
        "SELECT AVG(t.minlevel) minlevel, AVG(t.maxlevel) maxlevel FROM creature "
        "c "
        "INNER JOIN creature_template t ON c.id1 = t.entry WHERE map = {} AND "
        "minlevel > 1 AND ABS(position_x - {}) < {} AND ABS(position_y - {}) < "
        "{}",
...
uint32 RandomPlayerbotMgr::GetEventValue(uint32 bot, std::string const event)
{
    ...
    /*if (e.IsEmpty())
    {
        QueryResult results = PlayerbotsDatabase.Query("SELECT `value`, `time`,
    validIn, `data` FROM playerbots_random_bots WHERE owner = 0 AND bot = {} AND
    event = {}", bot, event.c_str());
...
bool RandomPlayerbotMgr::HandlePlayerbotConsoleCommand(ChatHandler* handler, char const* args)
{
    ...
    for (std::map<std::string, ConsoleCommandHandler>::iterator j = handlers.begin(); j != handlers.end(); ++j)
    {
        ...
        {
            uint32 account = *i;
            if (QueryResult results = CharacterDatabase.Query("SELECT guid FROM characters WHERE "
                                                              "account = {} AND name like '{}'",
                                                              account, name.c_str()))
            {
                do
                {
    ...
    // sRandomPlayerbotMgr->HandlePlayerbotCommand(args); for
    // (std::vector<std::string>::iterator i = messages.begin(); i !=
    // messages.end(); ++i)
    // {
...
void RandomPlayerbotMgr::OnPlayerLogin(Player* player)
{
    ...
    if (botsNearby > 100 && false)
    {
        ...
        // botPos.GetReachableRandomPointOnGround(player,
        // sPlayerbotAIConfig->reactDistance * 2, true);

        // player->TeleportTo(botPos);
        // player->Relocate(botPos.coord_x, botPos.coord_y, botPos.coord_z,
        // botPos.orientation);
        ...
        // player->Relocate(botPos.getX(), botPos.getY(), botPos.getZ(),
        // botPos.getO());
    }
...
void RandomPlayerbotMgr::PrintStats()
{
    ...
    /*sort(questCount.begin(), questCount.end(), [](std::pair<Quest const*, int32>
    i, std::pair<Quest const*, int32> j) {return i.second > j.second; });
    ...
    {
        LOG_INFO("playerbots", "    [{}]: {} ({})", quest.second,
    quest.first->GetTitle().c_str(), quest.first->GetQuestLevel()); cnt++; if (cnt
    > 25) break;
    }
    */
}
